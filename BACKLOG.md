...
- Operator handoff builder packages the release archive, deploy preflight, verified deploy request set, optional imported operator receipts, receipt checks, metrics report preflight, unsigned oracle tx request, and optional verified oracle tx result into a public handoff directory while preserving real blocker status; release-readiness audit validates the handoff package and keeps rollout blocked until external evidence exists.
