...
- GH-161 implementation is complete in protected PR #162 with exact-main publication pending. It adds canonical exact-byte owner-local model-directory manifests and capture-time re-verification before model adapter construction. This closes caller-only local digest derivation, but not upstream authenticity, live served-model identity, semantic quality, calibration, or production authorization.
