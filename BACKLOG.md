...
- GH-147 is a locally verified candidate: an exact canonical owner-only membership source now binds sorted unique Guardian IDs one-to-one to public BIP340 keys, fixed 8B tier, and model-artifact digests, then derives the existing snapshot and signer mappings. External source authority, key ownership/rotation, Sybil resistance, multi-host evidence, L1 attestation, protected review, and exact-main verification remain open.
