...
| 10B — Guardian Decentralization | IN PROGRESS | GH-33, GH-36, and GH-39 provide fail-closed hybrid analysis, complete 5+ Guardian voting, and BIP340-authenticated replay-safe intake. Merged and exact-main-verified GH-42/GH-44/GH-48/GH-52 provide the bounded ballot carrier, persistent transport identity, relay/AutoNAT operation, packaged sidecar, and explicit bootstrap routes. Merged and exact-main-verified GH-55 adds the canonical 2048-byte-bounded `/prometheus/threat-hint/1.0.0` core and development-safe Light Client builder; merged and exact-main-verified GH-58 adds its separate owner-only verifier IPC, trusted network/domain binding, persistent freshness/replay admission, and atomic durable analyzer outbox. The production verifier remains unavailable until an independently approved Groth16 relation, verifying key, and vectors exist, so operated acceptance stays fail-closed as `busy`. `PeerId` remains transport metadata only. Real two-host operation, broad discovery, trusted membership/key assignment, Sybil protection, real accepted ThreatHint analysis, live model wiring, on-chain attestation, and production operation remain open. |
...
**Progress estimate (2026-07-23):** the isolated H-001 canary preparation is about **96% complete**; only the explicitly approved external signature, verification, one-shot broadcast, confirmation, receipt, and independent evidence remain. A rollout-capable core network is about **76–80% complete** with the operated Guardian sidecar, explicit relay-bootstrap configuration, bounded ThreatHint transport, and durable fail-closed verifier ingress implemented; six state-contract deployments, the independently approved real Groth16 verifier and accepted analysis, PROM emission, real metrics-oracle execution/evidence, real two-host discovery/relay evidence, remaining P2P paths, and production node evidence remain open. The complete roadmap vision is about **42–47% complete**, so **53–58% remains**. These are scope-weighted engineering estimates, not calendar or release guarantees.
