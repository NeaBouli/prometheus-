...
Public project status was reviewed through 2026-09-13. The immutable claim-audit baseline remains 2026-08-14; later exact-main updates, including GH-253 and the planned GH-258/GH-261 safety track, are recorded below and in the machine-readable post-audit ledger.
...
### Planned endpoint detection and safe response

[GH-258](https://github.com/NeaBouli/prometheus-/issues/258) and its
[GH-261](https://github.com/NeaBouli/prometheus-/issues/261) clarification track
a future, evidence-gated endpoint-security layer for coordinated compromise,
including unauthorized compute conscription: attempts to commandeer endpoints,
accelerators, servers, or data-center capacity into a distributed compute mesh.
Detection would use observable process, file, network, persistence,
credential-access, model/tool, unexpected CPU/GPU workload,
scheduler/orchestrator, workload-identity, and bounded cross-device correlation
signals. Prometheus cannot reliably attribute an attack to AI, AGI, a specific
actor, or intent from those signals alone.

The planned order is: **observe only**, **warn only**, **operator-confirmed
reversible containment**, then separately approved **limited automation** after
independent privacy, false-positive, rollback, adversarial, resource, and
multi-host evaluation. No real-time endpoint sensor or response engine is
implemented today. Automatic process termination, quarantine, firewall changes,
...
| 9 — Deploy Path | H-001 CANARY CONFIRMED / FULL ROLLOUT BLOCKED | The closed, manifest-bound Testnet-10 `ValidatorStakingH001` canary was externally signed, fully verified, broadcast once, confirmed, and independently observed on 2026-08-12. Its public `operator_record` and evidence guards pass, but this non-promotable canary cannot authorize Mainnet or full rollout. The remaining six state deployments, real `reportMetrics` state/sponsor inputs and signatures plus confirmed successor evidence, production proof approval, multi-host operation, and final release hardening remain required. |
...
**GH-216 local binary evidence:** Merged and exact-main-verified GH-216/PR #217 at `13c1812` adds a test-only real-`prometheus-client` loopback E2E harness for offline/connected preflight, private checkpoint commit, SIGTERM/SIGINT drain, restart exact replay, rollback/equivocation rejection, and malformed, timed-out, or disconnected peers. CI `31978132036`, Security `31978132044`, and Pages `31978131647` pass. This is local Development evidence only, not public Testnet operation, independent RPC/IPFS truth or availability, deployment, Mainnet, or production readiness.

**GH-226 Light Client v1 ThreatHint submission (Development-only):** The real `prometheus-client` binary now reuses the existing Guardian P2P stack to send one exact canonical v1 ThreatHint over a single owner-configured literal-loopback QUIC route. Strict private files, an offline non-mutating preflight, persistent owner-only transport identity, a 1–60 second bound, redacted JSON, and accepted/duplicate/rejected/busy/transport-failure mapping are tested against a real same-host Guardian library and owner-only mock verifier. PR #227 merged as exact main `6c39af5`; Prometheus CI `32675287618`, Security Audit `32675287530`, and Pages `32675287300` pass. Beta and Mainnet reject before network activity. This is repository and same-host engineering evidence only: it adds no proof generation or proof authority, reporter membership, discovery, relay, retry, public/multi-host operation, wallet, chain write, reward, deployment, or production readiness.

**GH-229 controlled distinct-host evidence (Development/Testnet-10 only):** Merged PR [#230](https://github.com/NeaBouli/prometheus-/pull/230), exact main `fba8bb4`, preserves loopback as the default and adds one explicit `controlled-remote-testnet10` route for a single canonical direct literal-IP/UDP/QUIC-v1 Guardian peer. Unsafe, mapped, reserved, DNS, TCP, relay and mismatched routes fail closed; Beta and Mainnet still reject before identity mutation or networking. At `2026-08-26T23:36:04Z` (2026-08-27 operator-local), one operator-attested run from source commit `27e8b02` delivered one canonical hint between two distinct controlled hosts: sender and Guardian both recorded non-authorizing `rejected`, with zero retries, no persistence and acknowledgement authority `none`. The temporary sender-restricted UDP rule was removed immediately; no listener or public service remains. The [redacted evidence record](docs/evidence/gh-229-controlled-two-host-2026-08-27.json) does not independently prove host separation and does not establish public networking, relay/v2 operation, proof or membership authority, deployment, Mainnet or production readiness.

**GH-234 Light Client ThreatHint-v2 submission (Development/Testnet-10 only):** Merged and exact-main-verified GH-234/PR [#235](https://github.com/NeaBouli/prometheus-/pull/235), code commit `b450740`, exact main `f146fb2`, adds one separate one-shot sender for an owner-prepared canonical `ThreatHintV2TransportPayload`. The client parses the shared proof-envelope, Observable Bundle, approval-shape and report-nonce framing against separately trusted `testnet-10` before identity or network activity, then sends exactly once over `/prometheus/threat-hint/2.0.0`. Owner-only files, offline preflight, accepted/rejected/busy/transport-failure mapping, redacted output, Beta/Mainnet rejection and v1 regression are covered locally, including a real same-host binary/QUIC/Guardian test. Exact-main CI `33272578070`, Security Audit `33272577951`, and Pages `33272577407` pass. This verifies a repository development boundary only: the client does not generate proofs, verify approval authority, establish membership, run models/YARA, retry or persist, and it adds no public multi-host, wallet, chain, reward, deployment, Mainnet or production claim.

**GH-238 controlled distinct-host ThreatHint-v2 preparation (repository-only):** Merged and exact-main-verified GH-238/PR [#239](https://github.com/NeaBouli/prometheus-/pull/239), exact main `912d96d`, implements and locally tests repository-only preparation for one later controlled distinct-host Development/Testnet-10 ThreatHint-v2 attempt. The tooling uses challenge-bound, role-specific operator attestations over the source commit, the actual executable digest, the exact canonical payload digest, the exact v2 protocol, one shared observed UTC time, the actual rejected status, one attempt, zero retries and no persistence, with strict owner-only/no-symlink files, the exact 9,265-byte Rust wire bound, atomic no-clobber record output, a closed redacted verifier and CI test wiring. Exact-main CI `33279351831`, Security Audit `33279351822`, and Pages `33279351387` pass. No real GH-238 remote run has occurred and no GH-238 evidence record exists; host separation is not independently proven. This repository preparation is not a deployment or remote demonstration and adds no port, firewall, host, IAM, wallet, chain, deployment, Mainnet or production action or authority; a later real run requires separate explicit authorization.
...
**GH-161 local model artifact provenance (merged and exact-main verified):**
the preferred candidate-capture
...
Protected PR #162 squash-merged normally as exact main `d468426`; Prometheus
CI `31340112225`, Security Audit `31340112204`, and Pages `31340111625` pass on
that SHA.
...
**GH-246 signed Guardian membership continuity (merged and exact-main
verified):** one exact owner-only policy pins the network, a public
...
**GH-253 Guardian transition-authority rotation (merged and exact-main verified):**
...
**Merged and exact-main-verified GH-152 governed ThreatHint v2 identity hardening (not production-deployed):** schema v5 permanently records the exact statement digest, approval ID, and observable commitment as a database-enforced one-to-one-to-one pairing in the same atomic promotion transaction. Reusing any member with a fresh counterpart is a closed replay even after outbox or result retention. A v4 ledger migrates only when its outbox and result tables are empty; authority, replay high-water, and approval-consumption state are preserved. Any retained v4 outbox or result row remains unchanged and fails closed. Legacy and governed non-outbox consumption keep their prior behavior. PR #153 published exact main `3d203aa`; Prometheus CI `31306353671`, Security Audit `31306353670`, and Pages `31306353328` pass. This adds no proof authority, analyzer output, transport, disclosure, wallet, chain, reward, deployment, or production effect.

**Merged and exact-main-verified GH-167 bounded ThreatHint v2 transport substrate (not production-deployed):** Rust and Python now share one exact canonical transport frame and valid/invalid corpus for the v2 proof-envelope, Observable Bundle, approval wire, and untrusted report-nonce lookup key. The independent `/prometheus/threat-hint/2.0.0` libp2p channel reparses every inbound frame against an explicitly trusted local network before owner-only IPC. The Python ingress reparses again, resolves the nonce only through trusted active-session state, obtains trusted time locally, and then calls the existing governed promotion boundary with the original wires. Global request/stream budgets, strict accepted/rejected/busy acknowledgements, adversarial tests, and real separate-process same-host delivery evidence are included. PR #168 published exact main `7c62608`; Prometheus CI `31645624623`, Security Audit `31645624601`, and Pages `31645623547` pass. This closes the repository transport substrate only: it does not approve production relation/key/ceremony artifacts, authorize semantic/actionable analysis or disclosure, run LLM/YARA, prove public multi-host operation, or perform wallet, chain, reward, deployment, or external effects.

**GH-170 merged and exact-main verified - bounded YARA-X validation (not production-deployed):** Guardian candidate rules now pass a real compile-only `yara-x==1.4.0` boundary instead of substring checks. The in-memory validator accepts exactly one bounded ASCII rule, disables includes, rejects imports, multiple rules, compiler errors and warnings, and never scans a file, process, or data buffer. PR #171 published exact main `8d8e29c`; Prometheus CI `31650123073`, Security Audit `31650123055`, and Pages `31650122593` pass. This is structural syntax hardening only; it does not establish semantic detection quality, invoke a model, make a rule actionable, publish or submit anything, or grant production authority.

**GH-173 merged and exact-main verified - deterministic non-actionable semantic draft (not production-deployed):** The governed v2 worker can deterministically map already approved local `api_import` and `byte_pattern` observables into one bounded memory-only YARA draft, compile-check it through the GH-170 boundary, and atomically retain only exact input bindings, per-kind counts, a nonce-bound candidate-binding SHA-256, and the compile verdict. `file_sha256` values are counted but never embedded, existing v1 results remain readable, and no rule source, model output, confidence, `should_submit`, scan, disclosure, publication, wallet, chain, reward, or deployment effect is stored or triggered. PR #174 published exact main `1107b11`; Prometheus CI `31654308969`, Security Audit `31654308964`, and Pages `31654308875` pass. This does not establish semantic quality, actionable analysis, model calibration, or production authority and does not change rollout estimates.

**GH-177 merged and exact-main verified - isolated synthetic YARA semantic-quality gate (not production-deployed):** A standalone offline evaluator compiles one fixed GH-173-shaped synthetic rule with pinned `yara-x==1.4.0` and scans only 20 bounded, deterministically generated in-memory buffers. Its canonical report binds the exact corpus, policy, evaluator bytes, engine version, rule digest, confusion counts, and precision/recall/specificity; the committed baseline is 10 true positives, 10 true negatives, and zero false results under a strict 100% development gate. Authority is fixed to `none`. The module is not imported by worker, outbox, result, model, transport, submission, wallet, chain, reward, or deployment paths. This is not real-world semantic quality, actionable analysis, calibration, certification, or production authority. PR #178 merged as exact main `396d347`; CI, Security Audit, and Pages pass on that SHA. Rollout estimates are unchanged.

**GH-180 merged and exact-main verified - deterministic offline ThreatHint-v2 pipeline integration gate (not production-deployed):** Eight POSIX-only integration tests drive canonical synthetic transport bytes through the real Python ingress, governed promotion, schema-v5 atomic acceptance/outbox, bounded worker, and durable GH-173 non-actionable semantic-draft result. They prove exact statement, approval, observable, nonce, candidate, and result bindings plus fail-closed malformed/oversized input, replay/restart, duplicate concurrency, lease recovery, redacted analyzer failure, and transactional completion rollback. The gate composes existing production classes but adds no product runtime or authority path, and GH-177 remains structurally unwired from the pipeline. Local evidence includes 171 adjacent tests; 1303 Guardian tests passed and 4 intentional live-model tests skipped. PR #181 merged as exact main `a28ad00`; Prometheus CI `31662874366`, Security Audit `31662874399`, and Pages `31662873670` pass on that SHA. No real sample, network service, model, scan, disclosure, submission, wallet, chain, reward, deployment, actionable authority, or production certification is involved. Rollout estimates are unchanged.
...
**Current public status (reviewed 2026-09-13; immutable audit baseline 2026-08-14):** Exact audit-base main `5cd13bf` and its CI/Security/Pages runs pass; later exact-main updates through GH-253 plus the planned GH-258/GH-261 safety track are recorded in this README and the machine-readable post-audit ledger. The isolated H-001 Testnet-10 canary is complete and independently evidenced, GH-167 closes only the bounded repository ThreatHint-v2 transport substrate, and GH-253 proves only owner-local dual-signed transition-authority succession. GH-258 and GH-261 are planning, not endpoint implementation, unauthorized-compute detection, or response authority. No Prometheus protocol component is proven production-deployed. Production proof/artifact approval, privacy-reviewed semantic/actionable analysis, operated multi-host transport, external or decentralized membership authority, remaining deployments, oracle evidence, PROM emission, and release gates remain open.
...
**Internal engineering estimate (confirmed unchanged 2026-09-13):** the isolated H-001 Testnet-10 canary work item is complete. The rollout-capable core is estimated at **84–88%** and the complete roadmap vision at **50–55%**. GH-258/GH-261 add planned scope but no completed capability, so the estimates do not advance. These are internal, scope-weighted engineering estimates, not evidence of production readiness, calendar progress, network decentralization, or release timing. The [2026-08-14 claim-audit baseline](docs/claim-audit-2026-08-14.md) lists the remaining evidence gates.
