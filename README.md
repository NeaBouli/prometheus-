...
**GH-180 local candidate - deterministic offline ThreatHint-v2 pipeline integration gate (not merged or production-deployed):** Eight POSIX-only integration tests now drive canonical synthetic transport bytes through the real Python ingress, governed promotion, schema-v5 atomic acceptance/outbox, bounded worker, and durable GH-173 non-actionable semantic-draft result. They prove exact statement, approval, observable, nonce, candidate, and result bindings plus fail-closed malformed/oversized input, replay/restart, duplicate concurrency, lease recovery, redacted analyzer failure, and transactional completion rollback. The gate composes existing production classes but adds no product runtime or authority path, and GH-177 remains structurally unwired from the pipeline. Local evidence passes 171 adjacent tests and the complete Guardian suite with 1303 passed/4 intentional live-model skips; protected PR, CI, merge, and exact-main verification remain pending. No real sample, network service, model, scan, disclosure, submission, wallet, chain, reward, deployment, actionable authority, or production certification is involved. Rollout estimates are unchanged.
