...
**GH-167 bounded ThreatHint v2 transport substrate (not production-deployed):** Rust and Python now share one exact canonical transport frame and valid/invalid corpus for the v2 proof-envelope, Observable Bundle, approval wire, and untrusted report-nonce lookup key. The independent `/prometheus/threat-hint/2.0.0` libp2p channel reparses every inbound frame against an explicitly trusted local network before owner-only IPC. The Python ingress reparses again, resolves the nonce only through trusted active-session state, obtains trusted time locally, and then calls the existing governed promotion boundary with the original wires. Global request/stream budgets, strict accepted/rejected/busy acknowledgements, adversarial tests, and real separate-process same-host delivery evidence are included. This closes the repository transport substrate only: it does not approve production relation/key/ceremony artifacts, authorize semantic/actionable analysis or disclosure, run LLM/YARA, prove public multi-host operation, or perform wallet, chain, reward, deployment, or external effects.
