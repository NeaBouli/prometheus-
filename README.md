...
| 2 — Client | DEVELOPMENT FOUNDATION | Kaspa RPC foundation, cache-only rule reader, custom YARA-style matcher, ZK stub, GH-190 exact-byte ingestion, GH-193 strict current-Silverc constructor-state decoding, GH-197 owner-pin manifest consistency, and GH-203/PR #204 development-only live Testnet-10 UTXO/virtual-DAA acquisition through one connected node. GH-205 composes those boundaries with a credential-free loopback-only IPFS gateway fetch, exact Raw-CIDv1 binding, and one atomic complete-snapshot scanner replacement. GH-207 adds an owner-local POSIX checkpoint that rejects verified-virtual-DAA rollback and same-order equivocation across restarts, with exact replay recovery and fail-closed file handling. GH-209 adds an explicit opt-in development coordinator with bounded retry, timeout, cancellation, single-flight admission, and non-sensitive status. GH-211 adds a strict canonical BIP340-authenticated complete-snapshot envelope against a separately owner-pinned x-only key, external nonzero minimum sequence, and separately trusted clock rechecked on every fetch. Merged and exact-main-verified GH-213/PR #214 at `bbe7efb` adds an operator-invoked Development/Testnet-10 `rule-sync preflight/run` CLI with private strict config, offline non-mutating preflight, loopback-IP-literal RPC/IPFS boundaries, redacted status, and signal cancellation; CI `31950806131`, Security `31950806118`, and Pages `31950805653` pass. It has no signer, key authority or rotation proof, persistent sequence authority, autonomous authority, wallet or chain write; independent RPC truth, transaction history, consensus finality, canonical manifest authority, IPFS availability/replication, production YARA, and operated P2P reporting remain open |
