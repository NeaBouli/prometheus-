...
**GH-152 governed ThreatHint v2 identity hardening (not production-deployed):** schema v5 permanently records the exact statement digest, approval ID, and observable commitment as a database-enforced one-to-one-to-one pairing in the same atomic promotion transaction. Reusing any member with a fresh counterpart is a closed replay even after outbox or result retention. A v4 ledger migrates only when its outbox and result tables are empty; authority, replay high-water, and approval-consumption state are preserved. Any retained v4 outbox or result row remains unchanged and fails closed. Legacy and governed non-outbox consumption keep their prior behavior. This adds no proof authority, analyzer output, transport, disclosure, wallet, chain, reward, deployment, or production effect.
