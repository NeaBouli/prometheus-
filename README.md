...
| 10B — Guardian Decentralization | IN PROGRESS | The merged/exact-main-verified GH-33 router runs 8B first and escalates below confidence `0.70` to 70B while preserving the `0.85` submission threshold. Merged/exact-main-verified GH-36 adds a local fail-closed 5+ Guardian vote validator with domain-separated candidate/snapshot commitments, a complete ballot, strict majority, and conservative `min(source, approvals)` confidence. Merged/exact-main-verified GH-39 adds a local transport-neutral BIP340 intake that binds every Guardian ID to an exact per-session x-only public key, strictly verifies canonical signed envelopes and freshness, and atomically persists replay/equivocation markers across restarts and concurrent submissions. Live model wiring/evidence, an actual P2P carrier with discovery/NAT traversal, a trusted membership source, Sybil protection, on-chain attestation, and production operation remain open. |
