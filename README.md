...
**GH-170 local candidate - bounded YARA-X validation (not yet merged or production-deployed):** Guardian candidate rules now pass a real compile-only `yara-x==1.4.0` boundary instead of substring checks. The in-memory validator accepts exactly one bounded ASCII rule, disables includes, rejects imports, multiple rules, compiler errors and warnings, and never scans a file, process, or data buffer. This is structural syntax hardening only; it does not establish semantic detection quality, invoke a model, make a rule actionable, publish or submit anything, or grant production authority. Merge, protected checks, exact-main evidence, and live Pages verification remain pending.
