...
**GH-229 controlled remote evidence capability (Development/Testnet-10 only):** Merged PR [#230](https://github.com/NeaBouli/prometheus-/pull/230), exact main `fba8bb4`, preserves loopback as the default and adds one explicit `controlled-remote-testnet10` route for a single canonical direct literal-IP/UDP/QUIC-v1 Guardian peer. Unsafe, mapped, reserved, DNS, TCP, relay and mismatched routes fail closed; Beta and Mainnet still reject before identity mutation or networking. A committed non-authorizing receiver and strict redacted evidence gate can record only `rejected` or `busy`. Exact-main CI `33017195813`, Security Audit `33017196184`, and Pages `33017194744` pass, but the approved hosts currently have no direct UDP path, so no real two-host delivery or public operation is claimed. SSH/TCP tunneling, same-host processes and containers are not accepted as substitutes.
