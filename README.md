...
| 10B — Guardian Decentralization | IN PROGRESS | GH-33, GH-36, and GH-39 provide fail-closed hybrid analysis, complete 5+ Guardian voting, and BIP340-authenticated replay-safe intake. Merged and exact-main-verified GH-42 adds a real QUIC/libp2p direct request/response carrier for exact signed-ballot bytes, bounded connection/stream/request resources, static peer addresses, an owner-only Unix collector bridge, deterministic two-node transport, and end-to-end network-to-collector ACK tests. `PeerId` remains transport metadata only. GH-44 tracks operated relay/AutoNAT/DCUtR evidence and persistent transport identity; broad discovery, trusted membership/key assignment, Sybil protection, on-chain attestation, live model wiring, and production operation remain open. |
