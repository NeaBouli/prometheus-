...
| 10B — Guardian Decentralization | IN PROGRESS | GH-33, GH-36, and GH-39 provide fail-closed hybrid analysis, complete 5+ Guardian voting, and BIP340-authenticated replay-safe intake. Merged and exact-main-verified GH-42 adds the bounded QUIC/libp2p ballot carrier and owner-only collector bridge. Merged and exact-main-verified GH-44 adds atomic owner-only persistent transport identity, strict bounded direct/relay/AutoNAT routes, operator-safe health events, and a bounded relay service. Merged and exact-main-verified GH-48 packages guardian and relay roles behind strict owner-only TOML, local AF_UNIX submission, bounded path-free JSON health, and bounded SIGINT/SIGTERM drain. Merged and exact-main-verified GH-52 adds explicit canonical relay bootstrap advertisements and path-free routes while keeping bind addresses and transport authorization separate. Three process tests prove exact same-host relay delivery, ACK propagation, socket cleanup, stable identities, broken-output failure, and collector-wait shutdown. `PeerId` remains transport metadata only. Real two-host operation, broad discovery, trusted membership/key assignment, Sybil protection, on-chain attestation, live model wiring, and production operation remain open. |
...
**Progress estimate (2026-07-19):** the isolated H-001 canary preparation is about **96% complete**; only the explicitly approved external signature, verification, one-shot broadcast, confirmation, receipt, and independent evidence remain. A rollout-capable core network is about **73–77% complete** with the operated Guardian sidecar and explicit relay-bootstrap configuration implemented; six state-contract deployments, real Groth16, PROM emission, real metrics-oracle execution/evidence, real two-host discovery/relay evidence, remaining P2P paths, and production node evidence remain open. The complete roadmap vision is about **39–44% complete**, so **56–61% remains**. These are scope-weighted engineering estimates, not calendar or release guarantees.
