...
**Progress estimate (2026-07-28):** the isolated H-001 canary preparation is about **96% complete**; only the explicitly approved external signature, verification, one-shot broadcast, confirmation, receipt, and independent evidence remain. A rollout-capable core network is about **83–87% complete** with the operated Guardian sidecar, explicit relay-bootstrap configuration, bounded ThreatHint transport, durable ingress, real KIP-16-compatible Groth16 engine, fail-closed v1 analyzer adapter, and local review-ready v2 binding, preflight, real test-artifact verifier, verified-preflight composition, atomic local acceptance, owner-policy promotion/pairing, retention and enforceable authority/recipient/per-kind governance, plus the governed atomic recoverable outbox/claim boundary implemented. Production relation/key/ceremony approval and independent cryptographic review remain mandatory before that path can accept production proofs. Worker execution and real actionable v2 analysis, v2 transport, six state-contract deployments, PROM emission, real metrics-oracle execution/evidence, real two-host discovery/relay evidence, remaining P2P paths, and production node evidence remain open. The complete roadmap vision is about **49–54% complete**, so **46–51% remains**. These are scope-weighted engineering estimates, not calendar or release guarantees.
