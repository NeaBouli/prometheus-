...
- **Kaspa SDK** — kaspa-consensus-core, kaspa-rpc-core, kaspa-wrpc-client, kaspa-hashes, kaspa-addresses (rusty-kaspa)
- **Tokio** — Async Runtime
- **Serde / serde_json** — Serialisierung
- **sha2** — Kryptographie
- **Python** — Guardian-Node Tests (pytest, httpx), Scripts (autodidactic, audit_trigger)
- **Docker** — Guardian-Node Deployment (vLLM für LLaMA 3)
- **Silverscript** — 6 Smart Contracts auf Kaspa L1
- **HTML/JS** — Statische Website (index.html, whitepaper.html, faq.html, roadmap.html)
...
- **Client Entry:** `modules/client/src/main.rs`
- **Client Lib:** `modules/client/src/lib.rs`
- **Client Module:** `modules/client/src/` (ai/, blockchain/, network/, security/)
- **Validator Entry:** `modules/validator-node/src/lib.rs`
- **Validator Module:** `modules/validator-node/src/` (slashing/, voting/)
- **Contracts:** `modules/contracts/*.ss` (6 Silverscript-Contracts)
- **Guardian Node:** `modules/guardian-node/` (Docker, Python-Tests)
- **Client Tests:** `modules/client/tests/`
...
**Lies dies ZUERST bevor du Code anfasst:**
...
Schritt 2: Kontext laden (in dieser Reihenfolge!)
