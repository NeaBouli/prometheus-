...
- **Connectivity**: strict bounded IP/UDP/QUIC-v1 direct, relay-circuit, and explicit AutoNAT-server routes are implemented without DNS. The GH-44 isolated three-node harness proves relay reservation/delivery, AutoNAT state, DCUtR relay fallback, and disconnect handling; public or multi-host relay/NAT operation is not yet proven. mDNS remains excluded because the compatible optional DNS dependency currently carries unresolved RustSec advisories
...
GH-42 adds a real transport-only Guardian carrier. The GH-44 candidate extends
it with atomic owner-only persistent Ed25519 transport identity, strict bounded
direct/relay/AutoNAT routes, data-minimal health events, and a bounded relay
service. Its isolated three-node harness proves relay reservation, relay-only
ballot/ACK delivery, AutoNAT state, DCUtR failure with relay fallback, and
disconnect handling. A libp2p `PeerId`, static address, relay, or discovered
...
trustworthy, prove public or multi-host relay/NAT operation or broad discovery, prevent Sybil
