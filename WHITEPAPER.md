...
defines the separate artifact hash and observable commitment. GH-86 implements
isolated Rust and Python canonical bundle validators against one shared
byte-exact corpus. They prove structural and commitment consistency only and
are not connected to v1 transport, proof verification, Guardian analysis, or
rule publication. Reviewed kind-specific extractors, privacy gates, a v2 wire
and statement/relation, approved proof artifacts, and owner-only pairing remain
required before actionable analysis.
