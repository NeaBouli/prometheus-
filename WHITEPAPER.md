...
- The GH-213 repository candidate adds an explicit owner-invoked Development/Testnet-10 preflight/run CLI over the existing provider/coordinator/checkpoint path; offline preflight performs no network or checkpoint mutation, run accepts loopback-IP-literal RPC/IPFS only and emits redacted status, and no authority, wallet, chain write, deployment, Mainnet, or production claim is added
