...
**Status update — July 2026:** Kaspa Toccata is treated as a post-fork deployment environment for Prometheus. The current Silverc verifier covers H-001 commit-reveal byte encoding, ValidatorStaking runtime transitions, GuardianReputation runtime/formula gates, RuleStorage runtime gates, CommunityDonations runtime gates, DevIncentivePool runtime gates, and GovernanceAutoTuning runtime gates. Release-bundle, deploy-preflight, request/receipt/evidence verification, operator-handoff, metrics-oracle, and exact-commit release-hardening gates prepare operation without holding signing material. Closed deployment profiles separate the full seven-contract path from the non-promotable `testnet-10-validator-staking-h001` canary. The repository genesis operator closes transaction assembly, verification, broadcast, and observation while externalizing only BIP340 signing. Public testnet-10 funding and a deterministic schema-v2 H-001 request/digest rebuilt from exact main commit `9477fab` are confirmed; no signature or broadcast has occurred. Mainnet remains gated by the explicitly approved external canary signature, verified one-shot canary broadcast, confirmed public receipt plus independent evidence, the remaining contract deployments, the external metrics-oracle transaction, and public release-hardening evidence for the exact rollout commit.

**Genesis operator update — July 2026:** The repository contains `prometheus-silverc-deployer`, a keyless Toccata-v1 covenant-genesis operator pinned to official `rusty-kaspa` v2.0.1 APIs. It constructs transaction version 1 with compute budget 10 and the exact contextual `storage_mass` commitment, derives the official covenant ID, validates the exact live unspent funding UTXO during preflight and immediately before broadcast, and models the final 66-byte Schnorr signature script before exporting the 32-byte `SIG_HASH_ALL` digest. Signing-request schema v2 binds compute, transient, storage, normalized non-contextual/overall mass, the pinned relay rate, and both relay and conservative operator fee floors. The operator imports a plain public 64-byte BIP340 signature through a canonical lowercase-hex boundary, derives the response from the validated request, rejects normalized input/output path collisions, verifies BIP340 and the complete transaction before writing output, persists an exclusive intent before acknowledged broadcast, reconciles retry state by transaction ID, enforces per-request wRPC deadlines, and rebuilds the verified signed transaction before observing the resulting covenant UTXO. Thirty-eight unit/security tests include deterministic public interoperability values, fee/mass and profile tamper rejection, secret-field and resolver/TLS fail-closed coverage, signature-import failure/collision guards, journal recovery, and public-file handoff roundtrips. No private-key, seed, wallet, keystore, or raw-transaction input exists. Public testnet-10 funding plus an exact-main schema-v2 request/digest are confirmed. The H-001 canary still needs an explicitly approved external signature, complete operator verification, broadcast, confirmation, and independent chain evidence. Those results cannot authorize the full release or metrics-oracle readiness.
