...
**GH-173 local candidate - deterministic non-actionable semantic draft, August 2026 (not production-deployed):** One optional governed-worker analyzer deterministically derives a bounded YARA draft from already approved local `api_import` and `byte_pattern` observables, validates it only through the compile-only GH-170 boundary, and atomically stores a schema-v2 result containing exact input bindings, per-kind counts, a nonce-bound candidate-binding SHA-256, and compile status. The source remains memory-only; `file_sha256` values are count-only. Existing schema-v1 results remain readable. No model, confidence, `should_submit`, scan, disclosure, publication, wallet, chain, reward, deployment, or production authority is introduced. Protected merge is pending, and semantic quality, adversarial detection evidence, actionable-rule review, and production calibration remain open.
