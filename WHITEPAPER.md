...
**Genesis operator update — July 2026:** The repository now contains `prometheus-silverc-deployer`, a keyless Toccata-v1 covenant-genesis operator pinned to official `rusty-kaspa` v2.0.1 APIs. It constructs transaction version 1 with compute budget 10 and the exact contextual `storage_mass` commitment, derives the official covenant ID, validates the exact live unspent funding UTXO during preflight and immediately before broadcast, exports only the 32-byte `SIG_HASH_ALL` digest, enforces approved fee bounds, verifies an external BIP340 signature and the complete transaction, broadcasts only after exact request-hash acknowledgement, and observes the resulting covenant UTXO. Twenty-four unit/security tests include fixed public interoperability values and a public-file handoff roundtrip. No private-key, seed, wallet, keystore, or raw-transaction input exists. Real testnet-10 funding, signatures, confirmations, independent chain evidence, the metrics-oracle transaction, and exact-commit release evidence remain rollout gates.
