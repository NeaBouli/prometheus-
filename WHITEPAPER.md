...
- GH-211 adds a strict canonical BIP340-authenticated complete-snapshot envelope using a separately owner-pinned x-only key, external nonzero minimum sequence, and separately trusted per-fetch clock; it authenticates one owner-authorized request but proves no key authority/rotation, persistent sequence authority, canonical L1 state, RPC truth/finality, availability, deployment, or production readiness
