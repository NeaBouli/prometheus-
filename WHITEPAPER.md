...
Merged GH-229/PR #230 at exact main `fba8bb4` is a tested repository capability for one controlled Development/Testnet-10 direct-QUIC evidence run; exact-main CI `33017195813`, Security Audit `33017196184`, and Pages `33017194744` pass. Loopback remains the default; the explicit remote mode accepts one canonical static literal-IP peer and rejects unsafe/special ranges, mapped IPv6, DNS, TCP, relay and peer mismatches. Its committed local receiver can issue only non-authorizing `rejected` or `busy` outcomes and writes a redacted owner-only receipt. This is testable capability, not operated evidence: direct UDP between the approved hosts is currently unavailable, no substitute tunnel is accepted, and no two-host, public-network, proof, membership, deployment or production claim is enabled.
