...
GH-234/PR #235, code commit `b450740`, adds a separate Development/Testnet-10-only Light Client command that reads one owner-only canonical `ThreatHintV2TransportPayload`, reparses its shared framing against separately trusted `testnet-10` before identity or networking, and sends it once over the existing `/prometheus/threat-hint/2.0.0` Guardian channel. Local unit and real same-host binary/QUIC tests cover offline preflight, accepted/rejected/busy/transport-failure, one attempt, redacted output, fail-before-network Beta/Mainnet gates and unchanged v1 behavior. The payload remains owner-prepared: this client neither generates proof material nor verifies production approval authority, and the acknowledgement is not proof, privacy, membership, reward or chain authority. No public or multi-host v2 run, deployment, Mainnet or production operation is established.
