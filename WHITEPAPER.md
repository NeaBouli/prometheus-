...
- **Light Client AI target**: Phi-3-mini 3.8B (4-bit quantized, 4GB RAM, no GPU); current implementation is a bounded fail-closed safe-default stub with no loaded-model or quarantine authority
...
This section describes the target Light Client architecture. The current Rust client contains development implementations and fail-closed runtime guards: beta/mainnet reject the Phi-3 safe-default stub, SHA-256 ZK placeholder, cached rule reader, local CID-bound rule ingestion, manifest-bound RuleStorage observation verifier, and federated-learning placeholder. The Phi-3 stub creates no ONNX session, never treats file existence as a loaded model, accepts at most 16 MiB, and emits only a non-suspicious zero-confidence result with no quarantine authority. Merged and exact-main-verified GH-197/PR #198 can compare one separately owner-pinned canonical Testnet-10 manifest with a bounded caller-supplied RPC-shaped UTXO set, require a unique exact outpoint/covenant/script/amount/block-DAA match and a checked virtual-DAA maturity proxy, bind the exact GH-193 constructor JSON, and then decode it. GH-203/PR #204 adds a development-only adapter that obtains the current virtual DAA and UTXOs for one explicit `kaspatest` address from the existing connected client, requires the node to report exact Testnet-10, converts only pinned rusty-kaspa fields, and invokes that unchanged verifier. GH-205 composes that injected/live observation path with a credential-free loopback-only local IPFS gateway fetch, exact Raw-CIDv1 content binding, and one atomic complete-snapshot scanner replacement. GH-207 adds an owner-local POSIX checkpoint: the minimum verified observation virtual DAA is the conservative non-empty snapshot order, exact identities are digest-bound, lower orders and same-order equivocation fail closed, and exact replay restores scanner state after restart. GH-209 adds an explicit opt-in development coordinator around that transaction with bounded retry, timeout, cancellation, single-flight admission, and non-sensitive status. GH-211 adds a strict canonical BIP340-authenticated complete-snapshot envelope bound to exact Testnet-10, sequence, validity, entries, and empty-snapshot order; verification uses a separately owner-pinned x-only key, an external nonzero minimum sequence, and a separately trusted clock rechecked on every fetch. Merged and exact-main-verified GH-213/PR #214 at `bbe7efb` composes those callable APIs into an operator-invoked Development/Testnet-10 `rule-sync preflight/run` command with strict private local files, offline non-mutating preflight, loopback-IP-literal RPC/IPFS, redacted status, and signal cancellation; CI `31950806131`, Security `31950806118`, and Pages `31950805653` pass. It adds no signer, private-key path, key-authority or rotation proof, persistent sequence authority, autonomous rule authority, wallet, chain write, deployment, or Mainnet support. One signed owner-authorized snapshot envelope, one node response and one local gateway do not establish independent RPC truth, transaction history, consensus finality, canonical manifest authority, IPFS availability or replication, production YARA, or production readiness. These components must not be presented as a production threat-reporting pipeline.
...
Merged and exact-main-verified PR #227 published exact main `6c39af5`; Prometheus CI `32675287618`, Security Audit `32675287530`, and Pages `32675287300` pass. GH-226 adds one Development-only v1 ThreatHint submission boundary to the Light Client. The real binary reuses the existing Guardian P2P implementation in dial-only mode and accepts exactly one canonical static literal-loopback QUIC peer. Owner-only bounded files, offline non-mutating preflight, owner-only transport identity, exact canonical bytes, a 1–60 second bound, redacted status, and accepted/duplicate/rejected/busy/transport-failure outcomes are covered by real same-host binary/QUIC/Guardian-library evidence. Beta and Mainnet reject before any network activity. The remote acknowledgement is only a local Guardian-boundary outcome, not proof, consensus, membership, reputation, reward, chain or production authority. Public multi-host operation, discovery, proof generation/approval, privacy-reviewed reporting, deployment and production readiness remain open.

Merged GH-229/PR #230 at exact main `fba8bb4` adds the tested repository capability for one controlled Development/Testnet-10 direct-QUIC evidence run; exact-main CI `33017195813`, Security Audit `33017196184`, and Pages `33017194744` pass. Loopback remains the default; the explicit remote mode accepts one canonical static literal-IP peer and rejects unsafe/special ranges, mapped IPv6, DNS, TCP, relay and peer mismatches. At `2026-08-26T23:36:04Z` (2026-08-27 operator-local), one operator-attested run from source commit `27e8b02` delivered one canonical hint between two distinct controlled hosts. Sender and Guardian both recorded non-authorizing `rejected`, with zero retries, no persistence and acknowledgement authority `none`; the temporary sender-restricted UDP rule was removed immediately and no public listener remains. The [redacted evidence record](docs/evidence/gh-229-controlled-two-host-2026-08-27.json) does not independently prove host separation. It is not public-network or relay/v2 evidence and adds no proof, membership, privacy, model/YARA, chain, reward, deployment, Mainnet or production claim.

Merged and exact-main-verified GH-234/PR #235, code commit `b450740`, exact main `f146fb2`, adds a separate Development/Testnet-10-only Light Client command that reads one owner-only canonical `ThreatHintV2TransportPayload`, reparses its shared framing against separately trusted `testnet-10` before identity or networking, and sends it once over the existing `/prometheus/threat-hint/2.0.0` Guardian channel. Local unit and real same-host binary/QUIC tests cover offline preflight, accepted/rejected/busy/transport-failure, one attempt, redacted output, fail-before-network Beta/Mainnet gates and unchanged v1 behavior. Exact-main CI `33272578070`, Security Audit `33272577951`, and Pages `33272577407` pass. The payload remains owner-prepared: this client neither generates proof material nor verifies production approval authority, and the acknowledgement is not proof, privacy, membership, reward or chain authority. No public or multi-host v2 run, deployment, Mainnet or production operation is established.

GH-238 implements and locally tests repository-only preparation for one later controlled distinct-host Development/Testnet-10 ThreatHint-v2 attempt. The tooling uses challenge-bound, role-specific operator attestations over the source commit, the actual executable digest, the exact canonical payload digest, the exact v2 protocol, one shared observed UTC time, the actual rejected status, one attempt, zero retries and no persistence, with strict owner-only/no-symlink files, the exact 9,265-byte Rust wire bound, atomic no-clobber record output, a closed redacted verifier and CI test wiring. No real GH-238 remote run has occurred and no GH-238 evidence record exists; host separation is not independently proven. This repository preparation is not a deployment or remote demonstration and adds no port, firewall, host, IAM, wallet, chain, deployment, Mainnet or production action or authority; a later real run requires separate explicit authorization.
...
- Current implementation: fail-closed safe-default stub; no loaded-model, suspicion, malware-verdict, or quarantine authority; real ONNX inference remains open
...
- GH-203/PR #204 adds bounded development-only acquisition of those fields from one connected Testnet-10 node, with exact returned-address/network checks and generic errors; it does not independently authenticate the node response or prove finality, availability, or production authority
- GH-205 composes a complete owner-pinned snapshot through that observation boundary, a restricted local loopback IPFS gateway, repeated exact CID/content verification, and one atomic scanner swap
- GH-207 adds owner-local restart-persistent rollback/equivocation rejection for that development path using verified virtual-DAA ordering, canonical exact-identity digests, restrictive POSIX file handling, and exact replay recovery
- GH-209 adds an explicit opt-in development coordinator with one immediate attempt, a fixed interval after success, sequential retries with capped exponential failure backoff, bounded timeout, cancellation, single-flight admission and non-sensitive counters; it adds no CLI/product runtime, autonomous provider, canonical manifest authority, availability/replication proof, or production YARA
- GH-211 adds a strict canonical BIP340-authenticated complete-snapshot envelope using a separately owner-pinned x-only key, external nonzero minimum sequence, and separately trusted per-fetch clock; it authenticates one owner-authorized snapshot envelope but proves no key authority/rotation, persistent sequence authority, canonical L1 state, RPC truth/finality, availability, deployment, or production readiness
- Merged and exact-main-verified GH-213/PR #214 at `bbe7efb` adds an explicit owner-invoked Development/Testnet-10 preflight/run CLI over the existing provider/coordinator/checkpoint path; offline preflight performs no network or checkpoint mutation, run accepts loopback-IP-literal RPC/IPFS only and emits redacted status, and no authority, wallet, chain write, deployment, Mainnet, or production claim is added
- Merged and exact-main-verified GH-216/PR #217 at `13c1812` exercises that real CLI binary against test-only ephemeral loopback wRPC/IPFS peers, including private checkpointing, TERM/INT drain, restart replay and adversarial failures; it proves no public network, external availability, deployment or production operation
- Autonomous rules loaded from canonical L1 authority and available from replicated IPFS remain targets; PROM-RULES asset representation is also a deployment target
