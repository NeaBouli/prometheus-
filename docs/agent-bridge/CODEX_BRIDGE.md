...
- Do not continue Sprint 9/Mainnet deploy work until current-Silverc runtime, release-bundle manifest/archive gates, deploy preflight, external deploy requests, public deploy operator procedure, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, operator handoff packaging, release-readiness audit, public oracle tx-result verification, the missing network deploy/orchestration path, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and final release hardening are proven.
...
- Contracts: no Sprint 9 deploy until current-Silverc runtime, release-bundle manifest/archive gates, deploy preflight, external deploy requests, public deploy operator procedure, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, release-readiness audit, public oracle tx-result verification, the missing network deploy/orchestration path, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and final release hardening are proven
...
| 9 | Contract deploy on Testnet/Mainnet path | BLOCKED until the missing network deploy/orchestration path, real public deploy results/receipts, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and final release hardening pass. Current-Silverc runtime, release-bundle manifest/archive, deploy preflight, operator runbook, external deploy requests, public deploy operator procedure, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, operator handoff packaging, release-readiness audit, public metrics-oracle report preflight, unsigned oracle tx-request builder, and public oracle tx-result verifier gates pass locally; deploy operator procedure CI coverage is in the current working tree. |
...
- L-002 / Q-003: current-Silverc contract gate uses signed metrics `fp_rate` input; public metrics report preflight and unsigned tx-request builder pass locally and in GitHub Prometheus CI for `5fd385e`; external chain transaction assembly/signing/broadcast/deploy integration still needed.
