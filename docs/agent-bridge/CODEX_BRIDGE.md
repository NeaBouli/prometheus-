...
Operational branch: `main`; temporary bridge maintenance uses PR-only branches, and foreign untracked `Prometheus-1.png` remains untouched
...
Latest in-progress task: obtain an explicitly approved external BIP340 signature for the preserved exact GH-9 H-001 schema-v2 handoff without exposing private material, then perform full verification before any one-shot broadcast
