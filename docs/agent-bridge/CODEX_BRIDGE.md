...
Latest verified operational baseline: `178ad3e73866e43ada36c0e1b9e0892de510d8a1 docs: record GH-36 exact-main evidence (#38)`
Latest verified rollout/docs baseline: `178ad3e73866e43ada36c0e1b9e0892de510d8a1 docs: record GH-36 exact-main evidence (#38)`
Latest completed task: GH-36 and its documentation closeout are merged and exact-main verified. GH-39 is the current bounded Guardian task: local transport-neutral authenticated ballot intake plus persistent replay/equivocation protection, without changing the existing ensemble formula, contracts, chain behavior, wallets, or deployment gates.
Latest CI note: exact main `178ad3e` passed Prometheus CI `29462346299`, Security Audit `29462346239`, and Pages `29462345642`; all runs are bound to the same SHA. The GH-39 branch currently passes 117 Guardian tests with three intentional live-model skips, a focused 70-test signed-ballot/ensemble suite, Rustfmt, warning-free Clippy, 170 Rust workspace tests with two intentional live ignores, Black, focused Pylint 10.00/10, CI-scope Pylint 9.93/10, Python compilation, dependency audit with no known vulnerabilities, YAML parsing, Actionlint, Memory Integrity/Autodidactic, HTML parsing/markers, staged-scope Gitleaks, and diff checks. One first Rust run hit the documented sub-millisecond performance jitter at 1.408 ms; the isolated rerun and complete workspace rerun passed. Rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open.
Current PR note: issue https://github.com/NeaBouli/prometheus-/issues/39 tracks `feat/GH-39-authenticated-ballots`; its PR, protected checks, merge, exact-main verification, and live Pages verification remain pending. The branch binds Guardian IDs to exact per-session BIP340 x-only keys, strictly verifies canonical context/freshness envelopes, atomically persists one vote per Guardian and one nonce per active session in owner-only SQLite, and reverifies persisted ballots before the unchanged ensemble decision. Actual P2P carrier/discovery/NAT traversal, trusted membership/key assignment, Sybil resistance, on-chain attestation, and production operation are not claimed. Execution artifact baseline `205e1ca` and the GH-9 byte-identical signing request remain unchanged.
