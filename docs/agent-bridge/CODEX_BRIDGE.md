...
Current task: issue https://github.com/NeaBouli/prometheus-/issues/61 on branch `docs/GH-61-threat-hint-closeout` records the GH-58 exact-main and public-documentation closeout. It changes no product code. The next product gate is still an independently approved real Groth16/KIP-16 relation, verifying key, and test vectors plus an explicit analyzer-domain adapter; until those exist, the operated verifier remains unavailable/fail-closed as `busy` and cannot return `accepted`.
...
Latest verified public rollout/docs baseline: `22bc55a72bf441a2abc0372bc0eb789fb89fbb0b`; Prometheus CI `29962533693`, Security Audit `29962533720`, Pages `29962533075`, and live GitHub Pages build pass. Public README, Whitepaper, Roadmap, and `llms.txt` retain the explicit unavailable-verifier and accepted-analysis gates.
