...
Current local hardening note: deploy orchestrator-result imports and deployment receipt verification now reject raw/serialized transaction fields in addition to secret-like fields. Local py_compile, CI YAML parse, and deploy receipt import/verification smoke passed; remote CI verification is pending the next pushed commit.
