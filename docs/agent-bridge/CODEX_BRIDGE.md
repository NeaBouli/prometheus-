...
Operational baseline: exact main `34ab5b7a62b17ef2c9cab672439b77dcf4a66d9c` merged the GH-94 product implementation through PR #97 and passed Prometheus CI `29989116631`, Security Audit `29989117912`, and Pages `29989118948`. Raw GitHub README and live Whitepaper, Roadmap, FAQ, and `llms.txt` expose the merged local mandatory-review `byte_pattern` producer boundary. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/98 and branch `docs/GH-98-byte-pattern-producer-closeout` reconcile GH-94's merged exact-main and public evidence only. The product boundary remains one isolated Rust `byte_pattern` producer from exact caller-supplied artifact bytes, a checked start offset, a boolean wildcard mask, and typed scope. It accepts no path, pattern string, arbitrary observable value, or generic builder input; every fixed token comes from the selected byte range, the mask is bounded to 8..=64 positions, and at least eight positions must remain fixed. Output is always local-only `review_required_v1`, which does not authorize disclosure or transport. This establishes deterministic local extraction at the function boundary only: it does not prove external file provenance, maliciousness, semantic privacy approval, wildcard suitability, or proof binding and does not change v1 wire, P2P, verifier, analyzer, proof, wallet, signing, chain, KAS/PROM, slash ACL, commit-reveal, reputation, or emergency-stop policy. Separate rollout gates remain reviewed privacy/provenance promotion, a v2 statement/relation with independently approved artifacts, real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
Current GH-94 evidence: PR #97 passed all ten protected contexts on exact head `4284a88` and merged normally without admin bypass as `34ab5b7a62b17ef2c9cab672439b77dcf4a66d9c`; issue #94 closed. Exact-main Prometheus CI `29989116631`, Security Audit `29989117912`, and Pages `29989118948` pass. The automatic event dispatcher omitted the main suites, so the merged `workflow_dispatch` recovery trigger and existing Security trigger executed the complete workflows on exact main, while the Pages build API queued the exact-main Pages deployment; no check was fabricated or bypassed. Live README, Whitepaper, Roadmap, FAQ, and `llms.txt` markers pass. Earlier #95/#96 PR-ref failures remain documented. Independent Terra review found no blocking/high/medium issue; CodeRabbit's one valid corpus-documentation ambiguity was fixed. Complete local evidence remains 265 Rust tests plus one compile-fail doctest with two intentional live-network ignores, 181 Guardian tests with three intentional live-model skips, and all formatting, lint, package, Memory, HTML, workflow, dependency, and staged-secret checks. Claude Code was requested as a no-tool architecture helper, but its configured monthly spend limit rejected the call before analysis or repository access.
...
Latest completed task: issue https://github.com/NeaBouli/prometheus-/issues/94 is closed after PR https://github.com/NeaBouli/prometheus-/pull/97 merged normally. The local Rust producer derives one bounded mandatory-review `byte_pattern` from exact caller-supplied bytes, checked offset, boolean wildcard mask, and typed scope; Python independently validates the shared producer vectors. No path, pattern string, v2 wire, statement/relation, approved production relation, verifying key, proving key, analyzer promotion, wallet, signing, or chain path was added. `PeerId` remains transport metadata only. KAS/PROM roles, slash ACL, commit-reveal, reputation, and emergency-stop policy remain unchanged.
