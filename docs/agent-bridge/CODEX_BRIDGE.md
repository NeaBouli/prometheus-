...
Operational baseline: exact main `6659ab18a94f92d006fe24efe5a451d74322d1c6` merged the GH-82 closeout and passed Prometheus CI `29977755581`, Security Audit `29977755619`, and Pages `29977755074`. Raw GitHub Bridge and live Pages expose the merged design status. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/86 and branch `feat/GH-86-local-threat-observable-validators` now have locally complete local-only Rust/Python Canonical Observable Bundle v1 validation and one shared byte-exact vector corpus with 5 valid, 35 invalid-bundle, and 9 invalid-context cases. Rust ownership is isolated in `prometheus-threat-hint`; Python ownership is an unreferenced `jaeger` module. Neither is imported by the v1 client sender, P2P, verifier, durable ingress, analyzer, committee, IPFS, chain, or public-rule paths. The validators enforce structural grammar, canonical bytes, disclosure-policy syntax, and the exact network/nonce commitment, but cannot prove extractor provenance or semantic privacy; those remain separate reviewed producer gates. ThreatHint v1 and its proof formula remain unchanged. `review_required_v1` remains local-only. A v2 wire, new statement/relation, independently approved proof artifacts, owner-only pairing, and actionable analysis remain separate gates. Claude Code 2.1.218 completed a read-only retry after an earlier configured-budget stop and reported no blocking/high/medium finding; its one concrete low-severity test-coverage point is closed by a matching Rust grammar-before-policy regression. Spark and Terra supplied earlier bounded implementation/review while Sol retained architecture, hardening, and integration responsibility. Independent review findings for Rust deserialization bypass, value-bearing Debug output, Python post-parse mutation, and non-string disclosure-policy handling are closed; targeted re-review reports no remaining blocking/high/medium finding. CodeRabbit's protected review additionally aligned Python grammar-before-policy ordering with Rust and moved Rust comparison to the already-locked `subtle::ConstantTimeEq` primitive. Local evidence is 16 focused Rust tests plus one compile-fail doctest, 16 focused Python tests, 257 complete Rust workspace passes with two intentional live-network ignores, 179 complete Guardian Python passes with three intentional live-model skips, Rustfmt, warning-free workspace all-target Clippy, locked optimized builds, package checks, Black, Ruff, and Pylint. Refreshed PR/protected exact-main verification remains. Separate rollout gates remain real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
...
Latest verified operational product baseline: `4cada95ed2f97c2d0251dd82ef40290b0c664c41 fix: keep ThreatHint drains progressing (#79)`; Prometheus CI `29975041446`, Security Audit `29975041416`, and Pages `29975040944` pass. Latest verified design/public/status baseline is `6659ab18a94f92d006fe24efe5a451d74322d1c6` with CI `29977755581`, Security `29977755619`, Pages `29977755074`, and the merged GH-82 closeout.
Latest completed task: issue https://github.com/NeaBouli/prometheus-/issues/82 is closed after PR https://github.com/NeaBouli/prometheus-/pull/83 merged normally. No v2 wire, statement/relation, approved production relation, verifying key, proving key, or independent vectors are bundled, and no analyzer indicators are fabricated from the hash-only v1 transport schema. `PeerId` remains transport metadata only. Wallet/signing/chain behavior, KAS/PROM roles, slash ACL, commit-reveal, reputation, and emergency-stop policy remain unchanged.
