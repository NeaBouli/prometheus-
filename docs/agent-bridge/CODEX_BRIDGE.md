...
Branch observed: feature/GH-1-silverc-genesis-capability
Latest product-code baseline observed: `4236d27 ci: add silverc deploy preflight`
Latest verified tooling baseline observed: `40bb9a0 ci: verify release hardening evidence`
...
Current GH-1 note: PR https://github.com/NeaBouli/prometheus-/pull/2 at `57617ac` adds the official SilverScript covenant-genesis profile to the deploy procedure. External capability records must attest transaction version 1, `pay_to_script_hash_script` over the compiled contract script, official covenant-ID derivation from the funding outpoint and unbound outputs, and funding-input binding after ID derivation. Local py_compile, CI YAML parse, 55-test pinned Silverc runtime suite, seven-artifact release bundle, deploy request/procedure flow, positive capability verification, transaction-version/P2SH/covenant-ID/binding tamper checks, Rust workspace, and Clippy pass. All 10 PR checks pass, including Current Silverc Runtime, Gitleaks, cargo audit, and pip audit. Merge remains correctly blocked until an independent review is recorded; no admin bypass is used.
