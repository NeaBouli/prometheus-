...
Operational baseline: exact main `e7f34bb438d4d2cee43db9e8c019f05b9ced0f33` merged GH-90 through PR #91 and passed Prometheus CI `29984477087`, Security Audit `29984476876`, and Pages `29984476107`. Raw GitHub and live Pages expose the merged local `file_sha256` producer boundary. The earlier GH-86 product implementation merged through PR #87 as `2bfe5a3cd5df68bd9d17433748c06bb010070fae`; its evidence/documentation reconciliation merged through PR #89 as `4ce27b28473e8473922badd34b2e0e996d20b1a8`. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current status: GH-90 is complete, independently reviewed, merged, and exact-main verified. Issue https://github.com/NeaBouli/prometheus-/issues/92 records the documentation-only reconciliation of that evidence; no later autonomous product slice is recorded in this bridge. The producer accepts only typed scope plus exact caller-supplied bytes, computes the lowercase digest internally, and emits a bundle that passes the existing canonical validator. It accepts no path, caller-supplied digest, arbitrary observable value, or generic builder input. Rust proves exact production while Python independently validates the shared wire vectors. This is local deterministic derivation at the function boundary only: it does not prove that bytes came from a real file, establish maliciousness or semantic privacy, authorize transport, or change v1 wire, P2P, verifier, analyzer, proof, wallet, signing, chain, KAS/PROM, slash ACL, commit-reveal, reputation, or emergency-stop policy. Separate rollout gates remain reviewed privacy/provenance promotion, a v2 statement/relation with independently approved artifacts, real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
...
Latest verified operational product and public/status baseline: `e7f34bb438d4d2cee43db9e8c019f05b9ced0f33 feat: derive local file sha256 observables (#91)`; Prometheus CI `29984477087`, Security Audit `29984476876`, and Pages `29984476107` pass.
