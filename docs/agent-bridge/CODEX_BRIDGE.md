...
Latest active task: issue https://github.com/NeaBouli/prometheus-/issues/44 is implemented as a local candidate on `feat/GH-44-guardian-operated-transport`: atomic owner-only persistent transport identity, strict bounded direct/relay/AutoNAT routes, a bounded relay service, data-minimal health events, and isolated three-node relay/AutoNAT/DCUtR-fallback/disconnect evidence. Protected PR/merge, exact-main CI/Pages, public or multi-host operation, broad discovery, trusted membership/key assignment, Sybil resistance, and on-chain attestation remain open. `PeerId` is transport metadata only. The Bridge, README, Whitepaper, Roadmap, Backlog, `llms.txt`, Memory, module guide, and operator runbook are synchronized to this candidate boundary.
Latest CI note: exact main `c8038e93e1f3fdc4e7cb53e390cc8865d891eec1` passed Prometheus CI `29469319855`, Security Audit `29469319851`, and Pages `29469319460`; all runs are bound to the same SHA. Current GH-44 local evidence is 21 Guardian P2P crate tests; a clean full workspace rerun passes 191 Rust tests with two intentional live-network ignores; Guardian passes 126 tests with three intentional live-model skips; Rustfmt, workspace/all-target Clippy, Black, Pylint 9.95/10, Memory Integrity, six Autodidactic tests, five HTML parses, workflow YAML, Actionlint 1.7.12, `cargo audit` with no vulnerabilities and eight allowed maintenance/yank warnings, `pip-audit` with no vulnerabilities, and diff checks pass. One concurrent workspace attempt hit the known M-002 sub-millisecond benchmark jitter at 1.993 ms while Python 3.11 downloaded; isolated rerun passed at 914 microseconds and the clean full rerun passed. Independent review first caught a displaced validation block and then a production-only missing `rustix/fs` feature hidden by dev feature unification; both were fixed. Final re-review reports zero blocking/high/medium findings. PR, exact-main, and live Pages gates remain pending. Rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open.
