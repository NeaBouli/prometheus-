...
Latest verified operational baseline: `c8038e93e1f3fdc4e7cb53e390cc8865d891eec1 docs: close out guardian carrier rollout slice (#45)`
Latest verified rollout/docs baseline: `c8038e93e1f3fdc4e7cb53e390cc8865d891eec1 docs: close out guardian carrier rollout slice (#45)`
Latest active task: issue https://github.com/NeaBouli/prometheus-/issues/44 is implemented and pushed in PR https://github.com/NeaBouli/prometheus-/pull/46 on `feat/GH-44-guardian-operated-transport`: atomic owner-only persistent transport identity, strict bounded direct/relay/AutoNAT routes, a bounded relay service, data-minimal health events, and isolated three-node relay/AutoNAT/DCUtR-fallback/disconnect evidence. Protected merge, exact-main CI/Pages, public or multi-host operation, broad discovery, trusted membership/key assignment, Sybil resistance, and on-chain attestation remain open. `PeerId` is transport metadata only. The Bridge, README, Whitepaper, Roadmap, Backlog, `llms.txt`, Memory, module guide, and operator runbook are synchronized to this candidate boundary.
Latest CI note: PR #46 commit `840e71a5058ddf824701445fe6cc50857d796f98` passed all nine protected contexts in Prometheus CI `29470853630` and Security Audit `29470853695`; CodeRabbit passed and raised two valid documentation-precision comments, now addressed locally before refreshed checks. Current GH-44 local evidence is 21 Guardian P2P crate tests; a clean full workspace rerun passes 191 Rust tests with two intentional live-network ignores; Guardian passes 126 tests with three intentional live-model skips; Rustfmt, workspace/all-target Clippy, Black, Pylint 9.95/10, Memory Integrity, six Autodidactic tests, five HTML parses, workflow YAML, Actionlint 1.7.12, `cargo audit` with no vulnerabilities and eight allowed maintenance/yank warnings, `pip-audit` with no vulnerabilities, and diff checks pass. One concurrent workspace attempt hit the known M-002 sub-millisecond benchmark jitter at 1.993 ms while Python 3.11 downloaded; isolated rerun passed at 914 microseconds and the clean full rerun passed. Independent review first caught a displaced validation block and then a production-only missing `rustix/fs` feature hidden by dev feature unification; both were fixed. Final re-review reports zero blocking/high/medium findings. Merge, exact-main, and live Pages gates remain pending. Rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open.
Current GH-44 note: `transport_identity::load_or_create_transport_identity()` requires an absolute path and effective-user-owned mode-0700 parent, rejects symlinks and unsafe/noncanonical files, and atomically publishes a canonical Ed25519 identity from a synced mode-0600 same-directory temporary file. Strict config accepts only bounded IP/UDP/QUIC-v1 direct, exact relay-circuit, and explicit direct AutoNAT-server routes; DNS/mDNS and mismatched or unsafe dial routes fail. `RelayService` caps reservations, circuits, bytes, duration, connections, and per-peer use. Health events contain transport metadata only. No wallet, chain, contract, signing, reputation, KAS/PROM, slash ACL, commit-reveal, or Guardian authorization boundary changed. The operator runbook explicitly does not claim public/multi-host production operation or standalone packaging.
