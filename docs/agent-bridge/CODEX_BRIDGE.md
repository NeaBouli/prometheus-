...
- Do not continue Sprint 9/Mainnet deploy work until current-Silverc runtime, release-bundle manifest/archive gates, deploy preflight, external deploy requests, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, operator handoff packaging, the missing network deploy/orchestration path, external signed metrics-oracle transaction assembly/signing/deploy integration, and release hardening are proven.
...
- Contracts: no Sprint 9 deploy until current-Silverc runtime, release-bundle manifest/archive gates, deploy preflight, external deploy requests, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, the missing network deploy/orchestration path, external signed metrics-oracle transaction assembly/signing/deploy integration, and release hardening are proven
...
| 9 | Contract deploy on Testnet/Mainnet path | BLOCKED until the missing network deploy/orchestration path, external signed metrics-oracle transaction assembly/signing/deploy integration, and release hardening pass. Current-Silverc runtime, release-bundle manifest/archive, deploy preflight, operator runbook, external deploy requests, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, public metrics-oracle report preflight, and unsigned oracle tx-request builder gates pass locally and in CI. |
