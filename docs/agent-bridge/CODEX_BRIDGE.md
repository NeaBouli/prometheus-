...
Operational baseline: exact main `34ab5b7a62b17ef2c9cab672439b77dcf4a66d9c` merged the GH-94 local `byte_pattern` producer through protected PR #97 and passed exact-main Prometheus CI `29989116631`, Security Audit `29989117912`, and Pages `29989118948`. Issue #94 is closed. Raw GitHub and live Pages expose the merged `file_sha256` and mandatory-review `byte_pattern` boundaries. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/100 and branch `fix/GH-100-sidecar-signal-readiness` close a Guardian sidecar process race. The previous async `shutdown_signal()` did not install Tokio SIGINT/SIGTERM receivers until first poll, so the service could emit `waiting-for-collector` before SIGTERM handling existed and then exit through the operating-system default action. `run_service()` now constructs both Unix listeners before `OperatorOutput::start()`, and listener-registration errors fail before any readiness record. The crate is explicitly Unix-only because AF_UNIX and peer credentials are required. No protocol, transport authorization, ballot, verifier, analyzer, proof, wallet, signing, chain, KAS/PROM, slash ACL, commit-reveal, reputation, or emergency-stop boundary changes.
Current GH-100 evidence: the pre-fix compiled-binary stress reproduced the existing `sigterm_during_collector_wait_stops_cleanly` assertion at iteration 24; the patched binary passed 64/64 immediate post-readiness SIGTERM iterations. The focused regression passes, the complete three-test process suite passed ten consecutive runs, all 53 Guardian P2P unit tests plus all three process tests pass, and the complete Rust workspace passes with two intentional live-network ignores. Rustfmt, warning-free crate/workspace all-target Clippy, locked optimized Guardian build, verified ThreatHint package, dirty-tree Guardian package preview, Black, Guardian Pylint 9.86/10, and 181 Guardian tests with three intentional live-model skips pass. One non-reproduced relay-process `busy` result occurred in the first complete crate run; its isolated rerun, ten process-suite repetitions, and the later complete workspace run all passed, so no unproved backpressure change was made. The first Guardian Python run also hit one verifier-stub timeout; the focused rerun and complete rerun passed. Cargo Audit reports no vulnerabilities and the eight existing allowed warnings. Claude Code 2.1.218 independently reviewed the bounded patch without tools and found the Unix correction sound. Its question about lazy non-Unix registration is not applicable because the crate rejects non-Unix targets at compile time. Protected PR and exact-main evidence remain pending.
Documentation closeout issue https://github.com/NeaBouli/prometheus-/issues/98 remains open. PR #99 was closed without merge after GitHub left a successful runtime job in `in_progress`; its rerun exposed the GH-100 signal regression. After GH-100 merges and passes exact-main gates, recreate the GH-98 documentation reconciliation from the new main, update README/Whitepaper/Pages status, and merge it through normal protected checks.
...
Latest verified operational product and public/status baseline: `34ab5b7a62b17ef2c9cab672439b77dcf4a66d9c feat: derive review-required byte patterns (#97)`; Prometheus CI `29989116631`, Security Audit `29989117912`, and Pages `29989118948` pass.
