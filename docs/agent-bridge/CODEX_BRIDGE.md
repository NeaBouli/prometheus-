...
Branch observed: `fix/GH-17-genesis-fee-mass` from merged baseline `1e6e15c`
Latest merged baseline observed: `91cc13f docs: close GH-13 miner companion handoff (#15)`; latest merged product baseline remains `2e4b4ec Add safe experimental miner companion foundation (#14)`
Latest in-progress task: merge GH-17 signed-shape fee/mass hardening, rerun exact main CI, then rebuild the GH-9 H-001 request/digest from that merge without moving signing material into the repository
Latest CI note: main Prometheus CI `29423527697`, Security Audit `29423527568`, and Pages `29423526444` passed for merged baseline `91cc13fb33a2dab8734bd8617bd9fce5135447cc`. The live Whitepaper was verified on 2026-07-16. Current rollout issue https://github.com/NeaBouli/prometheus-/issues/9 is reopened for execution.
Current PR note: GH-13 implementation PR https://github.com/NeaBouli/prometheus-/pull/14 merged normally as `2e4b4ecc7c565f69700826122f15a5ff418b8cbe`; issue https://github.com/NeaBouli/prometheus-/issues/13 is closed. No direct-main push or admin bypass was used.
...
Current GH-9 note: PR #11 merged two closed deployment profiles bound to the exact deterministic release manifest. `full` retains all seven release fixtures, the public metrics-oracle key, and existing full-readiness statuses. `testnet-10-validator-staking-h001` selects only `ValidatorStakingH001`, requires `testnet` plus exact `kaspa-resolver://public`, forbids/omits the metrics-oracle key, and emits distinct canary request, procedure, receipt, evidence, and status values. The Rust loader pins manifest SHA-256 `e6cec2aa5d740c47c972fe92d4607ffd8a7a3c3f26b353475451d50b2670aefd` and exact full-contract order/count; CI cross-checks this constant against the generated Python profile. The H-001 profile cannot satisfy full handoff, production, or metrics-oracle readiness even after successful funding and confirmation. Public TN10 funding is now confirmed: deployer `kaspatest:qrj68xczazadtklg67f5yh39jzcq3fz3w6tvw4kv7xxl48ckc8cu7rwdr0jrt`, x-only key `e5a39b02e8bad5dbe8d793425e2590b008a4517696c756ccf18dfa9f16c1f1cf`, unspent outpoint `24e81339f3656689643ca86e3c53c4c5336e4273bb127d25bdaf328e5da241c7:0`, amount `100100000000` sompi, P2PK script `20e5a39b02e8bad5dbe8d793425e2590b008a4517696c756ccf18dfa9f16c1f1cfac`, and DAA `517772692`. The official TN10 API reports the transaction accepted, non-coinbase, unspent, and equal to the address balance. Remaining canary gates are exact-commit rebuild/request binding, external BIP340 digest signature, one-shot operator broadcast, confirmation, receipt, and independent public evidence. No metrics-oracle key is required for this first canary, and no private wallet data may enter the repository.
Current GH-17 note: the first real funding-bound dry run exposed that the old `80000`-sompi output and `2000`-sompi fee example produced `storage_mass=50000000` and did not enforce the pinned v2.0.1 post-Toccata fee/mass baseline. Branch `fix/GH-17-genesis-fee-mass` upgrades signing requests to schema v2, calculates the final 66-byte Schnorr-script shape, binds compute/transient/storage plus normalized non-contextual/overall mass, rejects unknown schema fields, and rejects fees below the pinned relay floor or Prometheus' conservative overall-mass floor before digest export. The signature integration test recomputes the final signed shape and checks every bound mass and fee floor. The mass-aware TN10 template uses a 10-TKAS covenant output, 500000-sompi fee, and change back to the same public deployer. A live candidate preflight passed with compute `2083`, transient `1412`, storage/overall `4001`, relay floor `208300`, and operator floor `400100`; 35 focused Rust tests pass. That branch candidate is not approved for signing and must be discarded/rebuilt from the exact merged commit after CI. No signature or broadcast occurred.
...
- Contracts: the repository-owned keyless genesis path passes locally and on merged-main CI; no Sprint 9 deploy until funded testnet-10 signatures and confirmed receipts, independent public node/explorer evidence, the external signed metrics-oracle transaction, and exact-commit release-hardening evidence are proven
...
| 9 | Contract deploy on Testnet/Mainnet path | IN PROGRESS on GH-17/GH-9 canary execution. The public testnet-10 P2PK outpoint is confirmed; fee/mass hardening must merge before the exact request/digest is rebuilt and sent for external BIP340 signing. Confirmation and public evidence then remain. Canary success cannot close Sprint 9. Full rollout remains blocked until the other six state-contract deployments, full-profile receipts/evidence, the signed metrics-oracle transaction, and exact-commit release hardening pass. |
...
3. Merge and remotely verify GH-17 fee/mass hardening, then bind the confirmed public TN10 P2PK outpoint to a freshly rebuilt exact-commit H-001 request and prepare its 32-byte digest.
4. Obtain an approved external BIP340 signature response without moving private key material into the repository, then execute only the `testnet-10-validator-staking-h001` canary and collect its confirmed `operator_record` receipt plus independent node/explorer evidence without promoting full status.
5. Execute the external signed metrics-oracle transaction before beta/mainnet governance.
...
Prometheus does not need reputation badges; it needs readable, provable Kaspa L1 Guardian reputation. All seven current-Silverc compile/ABI/runtime gates, the repository-owned keyless Toccata-v1 genesis operator, and the merged GH-7 TLS-only official-resolver probe pass local and remote CI. GH-9 separates a manifest-bound, non-promotable H-001 testnet-10 canary from the full seven-fixture rollout. Its public P2PK funding outpoint and matching public identity are confirmed; GH-17 fee/mass hardening must merge before the exact signing request is rebuilt. An external signature, canary confirmation, receipt, and public evidence then remain. The other contracts, metrics-oracle transaction, and exact-commit hardening still gate full rollout. Direct `ssh sandbox` remains unavailable from this Codex shell; no fallback secret is stored in the repo.
