...
Branch observed: `docs/GH-9-funded-outpoint` from merged baseline `91cc13f`
...
Latest in-progress task: rebuild the exact GH-9 H-001 canary execution artifacts/request, bind the confirmed public funding outpoint, prepare the digest, and obtain an approved external BIP340 signature response without moving signing material into the repository
...
| 9 | Contract deploy on Testnet/Mainnet path | IN PROGRESS on GH-9 canary handoff. The closed H-001 profile is ready locally but still needs a real funded testnet-10 P2PK outpoint, external BIP340 signature, confirmation, and public evidence. Canary success cannot close Sprint 9. Full rollout remains blocked until the other six state-contract deployments, full-profile receipts/evidence, the signed metrics-oracle transaction, and exact-commit release hardening pass. |
...
3. Bind the confirmed public TN10 P2PK outpoint to a freshly rebuilt exact-commit H-001 request and prepare its 32-byte digest.
