...
Branch observed: `feature/GH-13-miner-companion` from merged baseline `72c9717`
Latest merged baseline observed: `72c9717 docs: close GH-9 profile handoff (#12)`
Latest in-progress task: complete GH-13 experimental, opt-in Kaspa miner companion foundation without changing tokenomics or claiming production threat reporting
Latest CI note: main Prometheus CI `29413385980`, Security Audit `29413386091`, and Pages `29413385012` passed for merged baseline `72c97174bed742246964ce4570eaf98eec5cf827`. Current rollout issue remains https://github.com/NeaBouli/prometheus-/issues/9.
Current PR note: GH-13 implementation is on `feature/GH-13-miner-companion`; local full verification and independent reviews pass, and the pull request is the next step. Issue: https://github.com/NeaBouli/prometheus-/issues/13.
...
1. Complete GH-13 through review, CI, Pages verification, and normal PR merge while keeping its development-only boundary intact.
2. Keep the merged GH-9 profile and its full seven-fixture compatibility path green.
3. Obtain one public funded testnet-10 P2PK outpoint, matching public deployer identity, and an external vault/HSM BIP340 signature response without moving key material into the repository.
4. Execute only the `testnet-10-validator-staking-h001` canary, then collect its confirmed `operator_record` receipt plus independent node/explorer evidence without promoting full status.
