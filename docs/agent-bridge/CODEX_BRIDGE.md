...
Operational baseline: exact main `2bfe5a3cd5df68bd9d17433748c06bb010070fae` merged GH-86 and passed Prometheus CI `29981646898`, Security Audit `29981646867`, and Pages `29981646320`. Raw GitHub and live Pages expose the merged local-validator boundary. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/88 and branch `docs/GH-88-local-observable-closeout` reconcile the merged GH-86 evidence. PR #87 merged normally without admin bypass as exact main `2bfe5a3cd5df68bd9d17433748c06bb010070fae`; issue #86 is closed, all ten protected PR contexts passed, its only review thread is resolved, and exact-main CI/Security/Pages are green. The merged local-only Rust/Python Canonical Observable Bundle v1 validators consume one shared byte-exact corpus with 5 valid, 35 invalid-bundle, and 9 invalid-context cases. They enforce structural grammar, canonical bytes, disclosure-policy syntax, and the exact network/nonce commitment, but cannot prove extractor provenance or semantic privacy. ThreatHint v1 and its proof formula remain unchanged; neither implementation is imported by client sending, P2P, verifier, durable ingress, analyzer, committee, IPFS, chain, or public-rule paths. `review_required_v1` remains local-only. A reviewed extractor/privacy gate, v2 wire, new statement/relation, independently approved proof artifacts, owner-only pairing, and actionable analysis remain separate work. Local evidence is 16 focused Rust tests plus one compile-fail doctest, 16 focused Python tests, 257 complete Rust workspace passes with two intentional live-network ignores, and 179 complete Guardian Python passes with three intentional live-model skips. Separate rollout gates remain real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
...
Latest verified operational product and public/status baseline: `2bfe5a3cd5df68bd9d17433748c06bb010070fae feat: add local threat observable validators (#87)`; Prometheus CI `29981646898`, Security Audit `29981646867`, and Pages `29981646320` pass.
