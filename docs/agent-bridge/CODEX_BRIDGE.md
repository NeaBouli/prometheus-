...
Current handoff: GH-111 is complete and no product branch is active. The next internally buildable slice is a separate local canonical ThreatHint v2 wire/statement binding the artifact hash, observable commitment, report nonce, and network without transport, proof promotion, analyzer, wallet, or chain effects. Authority ownership/rotation, recipient-scope assignment, privacy approval, owner-only hint/bundle/approval pairing, approved relation/artifacts, actionable analysis, and crash-safe external side effects remain later gates.
