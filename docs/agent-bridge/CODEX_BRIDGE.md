...
Operational baseline: exact main `4ce27b28473e8473922badd34b2e0e996d20b1a8` merged the GH-86 evidence/documentation reconciliation through PR #89 and passed Prometheus CI `29982309546`, Security Audit `29982309489`, and Pages `29982308694`; the GH-86 product implementation itself merged through PR #87 as `2bfe5a3cd5df68bd9d17433748c06bb010070fae`. Raw GitHub and live Pages expose the merged local-validator boundary. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/90 and branch `feat/GH-90-local-file-sha256-producer` add one isolated Rust `file_sha256` producer from exact caller-supplied artifact bytes. The producer may accept only typed scope plus bytes, must compute the lowercase digest internally, and must emit a bundle that passes the existing canonical validator. It must not accept a path, caller-supplied digest, arbitrary observable value, or generic builder input. Shared vectors will let Rust prove exact production while Python independently validates the resulting wire. This is local deterministic derivation at the function boundary only: it does not prove that bytes came from a real file, establish maliciousness or semantic privacy, authorize transport, or change v1 wire, P2P, verifier, analyzer, proof, wallet, signing, chain, KAS/PROM, slash ACL, commit-reveal, reputation, or emergency-stop policy. Separate rollout gates remain reviewed privacy/provenance promotion, a v2 statement/relation with independently approved artifacts, real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
...
Latest verified operational product and public/status baseline: `4ce27b28473e8473922badd34b2e0e996d20b1a8 docs: close out GH-86 exact-main evidence (#89)`; Prometheus CI `29982309546`, Security Audit `29982309489`, and Pages `29982308694` pass.
