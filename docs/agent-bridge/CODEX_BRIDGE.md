...
| 9 | Contract deploy on Testnet/Mainnet path | BLOCKED until the missing network deploy/orchestration path, real public deploy results/receipts, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and final release hardening pass. Current-Silverc runtime, release-bundle manifest/archive, deploy preflight, operator runbook, external deploy requests, public deploy operator procedure, public orchestrator-result receipt import with raw-transaction rejection, deployment receipt verification, deployment status staging, operator handoff packaging, release-readiness audit, public metrics-oracle report preflight, unsigned oracle tx-request builder, public external-operator capability verifier, public oracle tx-result verifier, and public oracle status-draft staging gates pass locally and in CI for `181cde2`. |
