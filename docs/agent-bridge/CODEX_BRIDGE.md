...
Operational baseline: exact main `2ad2c44f37ec15ab2004b83daa5a8891945db1b3` merged the GH-90 evidence/documentation reconciliation through PR #93 and passed Prometheus CI `29985377659`, Security Audit `29985377455`, and Pages `29985377181`; the GH-90 product implementation itself merged through PR #91 as `e7f34bb438d4d2cee43db9e8c019f05b9ced0f33`. Raw GitHub and live Pages expose the merged local `file_sha256` producer boundary. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/94 and final protected branch `feat/GH-94-local-byte-pattern-producer-final` add one isolated Rust `byte_pattern` producer from exact caller-supplied artifact bytes, a checked start offset, a boolean wildcard mask, and typed scope. It accepts no path, pattern string, arbitrary observable value, or generic builder input; every fixed token comes from the selected byte range, the mask is bounded to 8..=64 positions, and at least eight positions must remain fixed. Output is always local-only `review_required_v1`, which does not authorize disclosure or transport. Shared vectors let Rust prove exact production while Python independently derives and validates the resulting wire. This establishes deterministic local extraction at the function boundary only: it does not prove external file provenance, maliciousness, semantic privacy approval, wildcard suitability, or proof binding and does not change v1 wire, P2P, verifier, analyzer, proof, wallet, signing, chain, KAS/PROM, slash ACL, commit-reveal, reputation, or emergency-stop policy. Separate rollout gates remain reviewed privacy/provenance promotion, a v2 statement/relation with independently approved artifacts, real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
Current GH-94 evidence: 18 focused Rust producer/validator tests and 18 focused Python observable tests pass. Shared vectors cover minimum, offset/wildcard, script, and exact 64-token/minimum-fixed boundaries; Rust regressions additionally cover invalid lengths, insufficient fixed bytes, out-of-range/exact-end/overflowing offsets, fixed-byte sensitivity, and wildcard invariance. Independent Terra review reports no blocking/high/medium finding. Its two coverage recommendations were implemented. Sol then found and closed a preallocation-order hardening gap by enforcing the 64-token/fixed-byte bounds before public-producer range selection or token collection as well as defensively in the constructor. The complete workspace passes 265 Rust tests plus one compile-fail doctest with two intentional live-network ignores; Guardian passes 181 tests with three intentional live-model skips. Rustfmt, warning-free workspace all-target Clippy, verified 15-file ThreatHint package contents, Black, Ruff, Guardian Pylint 9.86/10, Memory Integrity, six Autodidactic tests, HTML/JSON-LD/public-status checks, workflow YAML parsing, Actionlint 1.7.12, Cargo Audit with no vulnerabilities and eight known allowed warnings, staged-diff Gitleaks 8.30.1 with no leak, and clean diff checks pass. All ten initial protected PR #95 contexts passed on `6daa9f3`; CodeRabbit's only valid minor finding was ambiguous API-memory wording between the generic bundle corpus and the separate GH-94 producer corpus, now corrected without behavioral changes. GitHub registered later pushes but attached #95 and first replacement #96 check suites to stale heads; both PR envelopes were closed without merge. The final branch contains the identical reviewed product commit set plus documentation and a manual main-CI recovery trigger, and is opened without any post-open push so initial protected checks bind its exact final head. No required check is bypassed. Claude Code was requested as a no-tool architecture helper, but its configured monthly spend limit rejected the call before analysis or repository access. Final protected-PR checks and exact-main evidence remain pending.
...
Latest verified operational product and public/status baseline: `2ad2c44f37ec15ab2004b83daa5a8891945db1b3 docs: close out GH-90 exact-main evidence (#93)`; Prometheus CI `29985377659`, Security Audit `29985377455`, and Pages `29985377181` pass. The GH-90 product implementation itself remains exact main `e7f34bb438d4d2cee43db9e8c019f05b9ced0f33`.
Latest completed task: issue https://github.com/NeaBouli/prometheus-/issues/90 is closed after PR https://github.com/NeaBouli/prometheus-/pull/91 merged normally. The local Rust producer computes a single `file_sha256` from exact caller-supplied bytes and typed scope; Python independently validates the shared producer vectors. No path, v2 wire, statement/relation, approved production relation, verifying key, proving key, analyzer promotion, wallet, signing, or chain path was added. `PeerId` remains transport metadata only. KAS/PROM roles, slash ACL, commit-reveal, reputation, and emergency-stop policy remain unchanged.
