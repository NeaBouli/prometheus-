...
Branch observed: `main` at merged baseline `6213c559`
Latest merged baseline observed: `6213c559 Add manifest-bound H-001 testnet-10 canary profile (#11)`
Latest in-progress task: execute the non-promotable GH-9 `ValidatorStakingH001` testnet-10 genesis canary once public funding and external signing are available
Latest CI note: PR #11 merged normally. Main Prometheus CI `29412667386`, Security Audit `29412667410`, and Pages `29412666483` passed for exact baseline `6213c559508d3322b8660aed308df1a696ac5576`; the live whitepaper exposes the manifest-bound canary profile, 32-test operator baseline, and non-promotable boundary. Current rollout issue: https://github.com/NeaBouli/prometheus-/issues/9.
Current PR note: https://github.com/NeaBouli/prometheus-/pull/11 merged normally as `6213c559508d3322b8660aed308df1a696ac5576`. Its first Silverc job found a removed Full-status compatibility alias used by the external-capability verifier; the alias fix is included in the merge without weakening the distinct Canary status.
...
Current GH-9 note: PR #11 merged two closed deployment profiles bound to the exact deterministic release manifest. `full` retains all seven release fixtures, the public metrics-oracle key, and existing full-readiness statuses. `testnet-10-validator-staking-h001` selects only `ValidatorStakingH001`, requires `testnet` plus exact `kaspa-resolver://public`, forbids/omits the metrics-oracle key, and emits distinct canary request/procedure/receipt/evidence/status values. The Rust loader pins manifest SHA-256 `e6cec2aa5d740c47c972fe92d4607ffd8a7a3c3f26b353475451d50b2670aefd` and exact full-contract order/count; CI cross-checks this constant against the generated Python profile. The H-001 profile cannot satisfy full handoff, production, or metrics-oracle readiness even after successful funding and confirmation. Final local verification passes: 137 workspace tests with two intentional live-test ignores, warning-free workspace Clippy, Rustfmt, Python compile/Ruff, full seven-fixture and end-to-end canary regressions, Memory/Autodidactic, Actionlint, HTML/JSON-LD, diff hygiene, Gitleaks, and independent Terra re-review. Main Prometheus CI `29412667386`, Security Audit `29412667410`, and Pages `29412666483` pass for the merge commit. Real canary execution still requires a funded public testnet-10 P2PK outpoint, matching public deployer address/x-only key, external vault/HSM BIP340 signature response, confirmation, and independent public evidence. No metrics-oracle key is required for this first canary.
...
1. Keep the merged GH-9 profile baseline `6213c559` and its full seven-fixture compatibility path green.
2. Obtain one public funded testnet-10 P2PK outpoint, matching public deployer identity, and an external vault/HSM BIP340 signature response without moving key material into the repository.
3. Execute only the `testnet-10-validator-staking-h001` canary, then collect its confirmed `operator_record` receipt plus independent node/explorer evidence without promoting full status.
4. Execute the external signed metrics-oracle transaction before beta/mainnet governance.
5. Keep commit-reveal bounds and the no-badge/L1-reputation decision unchanged.
