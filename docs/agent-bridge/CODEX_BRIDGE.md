...
Branch observed: `feature/GH-7-public-resolver-probe`
Latest merged baseline observed: `4176093 Docs: close out GH-4 rollout tooling (#6)`
Latest in-progress task: GH-7 official public-resolver mode and funding-free testnet-10 Toccata probe
Latest CI note: merged baseline `4176093` passed Prometheus CI `29406057800`, Security Audit `29406057729`, and Pages `29406056965`. GH-7 commit `3cea782` is in PR https://github.com/NeaBouli/prometheus-/pull/8 and awaits remote CI/review.
...
| 9 | Contract deploy on Testnet/Mainnet path | BLOCKED until real funded testnet-10 signatures and confirmed receipts, independent public node/explorer evidence, the external signed metrics-oracle transaction, and exact-commit release-hardening evidence pass. Current-Silverc runtime, deterministic release handoff, the merged repository-owned keyless genesis path, public receipt/evidence/status guards, operator handoff/readiness, metrics-oracle handoff, and release-hardening gates pass locally and in CI. |
...
1. Merge GH-7 after remote CI and use its TLS-only resolver probe for the Hello-World testnet-10 operator run.
2. Obtain one public funded testnet-10 P2PK outpoint and an external vault/HSM BIP340 signature response without moving key material into the repository.
3. Collect confirmed `operator_record` receipt plus independent node/explorer evidence before expanding to all contracts.
...
Prometheus does not need reputation badges; it needs readable, provable Kaspa L1 Guardian reputation. All seven current-Silverc compile/ABI/runtime gates and the merged repository-owned keyless Toccata-v1 genesis operator pass local and remote CI. GH-7 adds a locally verified TLS-only official-resolver probe for synced, UTXO-indexed post-Toccata testnet-10 access without funding. The operator assembles, verifies, broadcasts, and observes official covenant deployments while delegating only digest signing to an external vault/HSM. Sprint 9 remains blocked until funded testnet-10 signatures and confirmed receipts, independent public node/explorer evidence, the external metrics-oracle transaction, and exact-commit release-hardening evidence pass. Direct `ssh sandbox` remains unavailable from this Codex shell; no fallback secret is stored in the repo.
