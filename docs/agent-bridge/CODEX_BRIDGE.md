...
Operational baseline: exact main `fceff1d3ae6db0f38c0076bc2c8dc82f34c3d96d` merged GH-82 and passed Prometheus CI `29977301070`, Security Audit `29977301063`, and Pages `29977300539`. Raw GitHub README/spec plus live Homepage, Whitepaper, Roadmap, FAQ, and `llms.txt` contain the merged Threat Observable v2 boundary. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/84 and branch `docs/GH-84-threat-observable-v2-closeout` reconcile the GH-82 merge evidence before the next product slice. PR https://github.com/NeaBouli/prometheus-/pull/83 merged normally without admin bypass as exact main `fceff1d3ae6db0f38c0076bc2c8dc82f34c3d96d`. ThreatHint v1 still carries a caller-supplied opaque hash; the protocol binds it but does not derive it from a file or observable. The merged v2 draft keeps `artifact_hash` and `observable_commitment` separate, defines strict canonical bounds and deny-by-default disclosure, and states that commitment matching proves byte consistency only, not truth, maliciousness, authorship, or artifact derivation. `review_required_v1` remains local-only and rejected at every IPC, P2P, analyzer, committee, IPFS, chain, and public-rule boundary until a separate authenticated approval protocol exists. All refreshed protected contexts passed and all review threads were resolved before merge. The next product slice is local Rust/Python bundle validation plus shared vectors; a v2 wire, new statement/relation, independently approved proof artifacts, owner-only pairing, and actionable analysis remain separate gates. Claude Code 2.1.218 was invoked as a read-only helper with no shell/write access but stopped before analysis at its configured USD limit; independent Terra/Explorer reviews supplied the architecture and privacy maps. Separate rollout gates remain real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
...
Latest verified operational product baseline: `4cada95ed2f97c2d0251dd82ef40290b0c664c41 fix: keep ThreatHint drains progressing (#79)`; Prometheus CI `29975041446`, Security Audit `29975041416`, and Pages `29975040944` pass. Latest verified design/public/status baseline is `fceff1d3ae6db0f38c0076bc2c8dc82f34c3d96d` with CI `29977301070`, Security `29977301063`, Pages `29977300539`, and live README/Homepage/Whitepaper/Roadmap/FAQ/`llms.txt`/spec checks.
