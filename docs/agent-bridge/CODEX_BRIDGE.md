...
Current task: issue https://github.com/NeaBouli/prometheus-/issues/55 on branch `feature/GH-55-threat-hint-transport` adds a separate canonical `/prometheus/threat-hint/1.0.0` core, an OS-independent shared schema, and a Light Client builder. Development proof stubs fail closed in beta/mainnet. The operated Guardian sidecar rejects every hint until a dedicated owner-only real-Groth16 verifier, freshness/replay policy, and bounded analyzer ingress exist. The ballot protocol and collector are not reused. `PeerId` remains transport metadata only; no Guardian/reporter authorization, wallet, signing, chain, reputation, KAS/PROM, slash ACL, or commit-reveal boundary changes.
Latest verified operational baseline: `f2e52beebe5ec7d6a3e6e0e8d36bced8f6f68ac7 feat: add explicit guardian relay bootstrap routes (#53)`
Latest verified public rollout/docs baseline: `114fb5a69722dd3630a1bb8537305358afb55ac6 docs: close out guardian bootstrap rollout (#54)`; Prometheus CI `29644661857`, Security Audit `29644661860`, Pages `29644661435`, and live GitHub README, Whitepaper, Bridge, and Pages markers pass
Latest completed task: issue https://github.com/NeaBouli/prometheus-/issues/52 is closed after PR https://github.com/NeaBouli/prometheus-/pull/53 merged normally. GH-52 separates relay bind listeners from strict canonical relay advertisements, registers only explicit advertisements as external addresses, emits path-free persistent-`PeerId` bootstrap routes, and documents controlled two-host operation. Same-host process evidence proves configured route use but is not public or multi-host evidence. Broad discovery, trusted membership/key assignment, Sybil resistance, on-chain attestation, and production operation remain open. `PeerId` is transport metadata only.
Latest CI note: final GH-52 product PR passed all ten protected contexts in Prometheus CI `29644104499` and Security Audit `29644104466`; CodeRabbit reported pass but was rate-limited without review threads, while the independent Terra review and recheck found no remaining blocker, high, or medium issue. Exact main `f2e52beebe5ec7d6a3e6e0e8d36bced8f6f68ac7` passed Prometheus CI `29644233106`, Security Audit `29644233098`, and Pages `29644232771`. Local evidence is 38 Guardian P2P unit tests plus three process tests; 211 workspace Rust tests with two intentional live-network ignores; 126 Guardian tests with three intentional live-model skips; Rustfmt; warning-free workspace/all-target Clippy; a locked optimized release build; a 13-file Cargo package; Black; Pylint 9.95/10; Memory Integrity; six Autodidactic tests; five HTML parses; dependency audit with no vulnerabilities and eight allowed warnings; Secret Detection/Gitleaks; public-status and diff checks. Rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open.
Current GH-55 verification note: PR https://github.com/NeaBouli/prometheus-/pull/56 passed its first complete protected CI/Security run. CodeRabbit found one major shared-admission defect: the ballot handler counted only ballot work while the ThreatHint handler counted both protocols. Both paths now use one shared inbound counter, and a mixed-protocol saturation regression proves the global cap. The same review patch aligns four minor documentation findings. Local post-fix evidence is 45 Guardian P2P unit tests plus three process tests, eight focused client tests, Rustfmt, and warning-free workspace/all-target Clippy; refreshed protected checks are required before merge.
