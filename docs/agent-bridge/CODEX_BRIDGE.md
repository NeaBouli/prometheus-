...
Operational branch: `fix/GH-9-canonical-signature-import`; product and bridge changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched
Latest verified rollout/docs baseline: `9e28629d81ca19a8716618034d5117922e8bfb8f docs: synchronize rollout readiness status (#21)`
Latest in-progress task: merge and exact-main verify the canonical public-signature import hardening, then obtain an explicitly approved external BIP340 signature for the preserved GH-9 H-001 schema-v2 handoff without exposing private material; full verification remains mandatory before any one-shot broadcast
Latest CI note: main Prometheus CI `29446061787`, Security Audit `29446061842`, and Pages `29446060342` passed for exact merged commit `9e28629d81ca19a8716618034d5117922e8bfb8f`. Live `index.html` and `roadmap.html` expose the readiness-gated 96% canary, 65-70% core-network, and 35-40% complete-vision estimates. Current rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open for execution.
Current PR note: GH-20 PR https://github.com/NeaBouli/prometheus-/pull/21 merged normally as `9e28629d81ca19a8716618034d5117922e8bfb8f`; issue https://github.com/NeaBouli/prometheus-/issues/20 is closed. CodeRabbit's only actionable chronology finding was fixed before merge. No direct-main push or admin bypass was used. The baseline above tracks the latest verified product/docs state; a later bridge-only commit does not invalidate it.
...
Current GH-9 note: PR #11 merged two closed deployment profiles bound to the exact deterministic release manifest. `full` retains all seven release fixtures and the public metrics-oracle key. `testnet-10-validator-staking-h001` selects only `ValidatorStakingH001`, requires exact `kaspa-resolver://public`, forbids the oracle key, and cannot promote full or metrics readiness. Public TN10 funding remains confirmed and unspent. After GH-17 merged, the release bundle, one-request canary set, funding spec, live node/funding preflight, and schema-v2 signing request were rebuilt from exact main commit `9477fabb8a9abb41e0ee82f7e240a99436452d2c` outside the repository. The live preflight resolved a synced, UTXO-indexed `rusty-kaspa 2.0.1` node above Toccata activation and revalidated the exact unspent, non-coinbase P2PK outpoint. The deterministic signing request binds request `c0cad33f23acfee4114092e0211dd642cb97c44891cc8f8826f4656f406f42fa`, signing-request hash `6b8e65065ca5ae2ca561ddd3fcb9659c384496fd31db32c137fcc9d811fa5323`, 32-byte sighash `174ccbe80d1d37e62d2bbabfbfba48245372df2bcf9e6724ac79ebc16b4e0bcd`, unsigned transaction ID `c85fd1e79607370ff63faabbc3158e011158d2bde1af40d4eecd642189d8c22f`, and covenant ID `8cdb49adc2511ed5fa11f71317b66f203f37ce376a2603cd9b118236c1f8219f`. It uses a 10-TKAS covenant output, 500000-sompi fee, compute `2083`, transient `1412`, storage/overall `4001`, relay floor `208300`, and operator floor `400100`. A second prepare produced byte-identical JSON and the public handoff directory passed Gitleaks. The reproducible public files are preserved outside Git at `/Users/gio/Desktop/repos/prometheus-handoffs/9477fab` with owner-only permissions. No signature or broadcast occurred. Remaining canary gates are an explicitly approved external BIP340 signature response, full operator verification, one-shot broadcast, confirmation, receipt, and independent public evidence.
...
Current GH-9 signature-import hardening note: branch `fix/GH-9-canonical-signature-import` adds `import-signature`, which accepts only a public 64-byte BIP340 signature as 128 lowercase hex characters with at most one trailing line ending. It derives every response field from the validated schema-v2 request, resolves paths before I/O, rejects output collisions with any input or other output, and writes response/verification files only after BIP340 plus full Kaspa transaction verification. The standard Kaspa wallet `message sign` command is explicitly forbidden for this digest because its personal-message domain hash is incompatible. Thirty-eight focused tests, warning-free Clippy, CLI smoke, YAML parsing, workspace tests, and an independent Terra review pass locally. No wallet, private material, signature, raw signed transaction, or broadcast was used. Merge and exact-main CI remain pending.
