...
- Do not continue Sprint 9/Mainnet deploy work until current-Silverc runtime, release-bundle manifest/archive gates, deploy preflight, external deploy requests, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, operator handoff packaging, public oracle tx-result verification, the missing network deploy/orchestration path, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and release hardening are proven.
...
- Contracts: no Sprint 9 deploy until current-Silverc runtime, release-bundle manifest/archive gates, deploy preflight, external deploy requests, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, public oracle tx-result verification, the missing network deploy/orchestration path, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and release hardening are proven
...
| 9 | Contract deploy on Testnet/Mainnet path | BLOCKED until the missing network deploy/orchestration path, real public deploy results/receipts, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and release hardening pass. Current-Silverc runtime, release-bundle manifest/archive, deploy preflight, operator runbook, external deploy requests, public orchestrator-result receipt import, deployment receipt verification, deployment status staging, public metrics-oracle report preflight, unsigned oracle tx-request builder, and public oracle tx-result verifier gates pass locally and in CI. |
