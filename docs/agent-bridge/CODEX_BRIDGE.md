...
Latest verified operational baseline: `b639a5bc80bf4f6e442e6dc295c091714089ce38 docs: close out guardian transport rollout slice (#47)`
Latest verified rollout/docs baseline: `b639a5bc80bf4f6e442e6dc295c091714089ce38 docs: close out guardian transport rollout slice (#47)`; product baseline remains GH-44 merge `27c2edc311f41f7eada4cad725b0e1ef29b83a51`
Latest active task: issue https://github.com/NeaBouli/prometheus-/issues/48 on branch `feat/GH-48-guardian-sidecar-service`. The locally verified candidate packages explicit `preflight`, `run`, and `submit` commands; strict owner-only role-tagged TOML; Guardian and relay owner loops; owner-only AF_UNIX submission; live listener readiness; path-free JSON lifecycle/health; bounded SIGINT/SIGTERM drain; collector-outage `busy`; and a separate-process same-host relay/receiver/sender/submit/collector proof. PR, protected checks, merge, exact-main, and live Pages evidence remain pending. Public or multi-host operation, broad discovery, trusted membership/key assignment, Sybil resistance, and on-chain attestation remain open. `PeerId` is transport metadata only.
Latest CI note: exact main `b639a5bc80bf4f6e442e6dc295c091714089ce38` passed Prometheus CI `29471846882`, Security Audit `29471846860`, and Pages `29471846338`. GH-48 local evidence is 32 Guardian P2P unit tests plus one separate-process test; 203 workspace Rust tests with two intentional live-network ignores; 126 Guardian tests with three intentional live-model skips; Rustfmt; warning-free workspace/all-target Clippy; a locked optimized release build; a 13-file Cargo package; Black; Pylint 9.95/10; Memory Integrity; six Autodidactic tests; five HTML parses; Actionlint 1.7.12; dependency audit with no vulnerabilities and eight allowed warnings; staged Gitleaks v8.30.1; public-status and diff checks. Independent review found one medium collector-ACK trailing-byte gap; strict EOF validation and a regression close it, and final security plus CI/package re-reviews report no remaining actionable findings. PR/protected checks/merge/exact-main/live evidence remain pending. Rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open.
Current GH-48 note: the service config and all IPC paths fail closed on unsafe owner/mode/symlink/type conditions and do not expose local paths or ballot/collector bytes in JSON. Local submission frames require canonical PeerId, bounded exact ballot bytes, and request/response EOF. Signal shutdown stops admission/listeners and drains admitted work to a bounded deadline. The same-host process test proves exact relay delivery, ACK propagation, socket cleanup, and stable transport identity but is not public or multi-host evidence. No wallet, chain, contract, signing, reputation, KAS/PROM, slash ACL, commit-reveal, or Guardian authorization boundary changed.
Current PR #49 note: feature commit `036935b` was pushed and the first run passed eight protected contexts plus CodeRabbit. Rust Workspace run `29474277765` alone failed because Linux reset a busy local connection whose request bytes were unread. The follow-up uses a separately bounded reject pool to consume and validate one exact frame before returning `busy`; focused and complete crate tests plus all-target Clippy pass locally. Refreshed PR checks are pending.
