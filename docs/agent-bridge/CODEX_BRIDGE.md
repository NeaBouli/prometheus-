...
Operational baseline: product exact main `4cada95ed2f97c2d0251dd82ef40290b0c664c41` passed Prometheus CI `29975041446`, Security Audit `29975041416`, and Pages `29975040944`. GH-80 docs/status exact main `e1756f966e66d92e00dbdeb161dd57641808ba4e` passed CI `29975474205`, Security `29975474202`, and Pages `29975473701`, with live GH-77 closeout markers. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/82, branch `docs/GH-82-threat-observable-v2-spec`, and PR https://github.com/NeaBouli/prometheus-/pull/83 freeze an evidence-backed Threat Observable v2 contract before implementation and correct unsupported privacy/proof claims. ThreatHint v1 carries a caller-supplied opaque hash; the protocol binds it but does not derive it from a file or observable. The preferred v2 keeps `artifact_hash` and `observable_commitment` separate, defines strict canonical bounds and deny-by-default disclosure, and states that commitment matching proves byte consistency only, not truth, maliciousness, authorship, or artifact derivation. `review_required_v1` is local-only and rejected at every IPC, P2P, analyzer, committee, IPFS, chain, and public-rule boundary until a separate authenticated approval protocol exists. This slice adds no wire, proof relation, analyzer invocation, or production acceptance. Local HTML/JSON-LD, public-status, link, YAML, Memory Integrity, six Autodidactic tests, and diff checks pass; final independent architecture/privacy reviews report 0 blocking/high/medium findings. Initial protected Prometheus CI `29976668964` and Security Audit `29976668927` passed all ten contexts, including Gitleaks, dependencies, and CodeRabbit. CodeRabbit supplied five valid consistency findings plus one Markdown lint nit; all are fixed locally, and refreshed protected checks/review remain pending. Local cross-language validators/vectors are next after protected merge; a new v2 statement/relation, independently approved artifacts, owner-only pairing, and actionable analysis remain separate gates. Claude Code 2.1.218 was invoked as a read-only helper with no shell/write access but stopped before analysis at its configured USD limit; independent Terra/Explorer reviews supplied the architecture and privacy maps. Separate rollout gates remain real two-host operation, H-001 external signing/execution evidence, six remaining deployments, metrics-oracle execution, PROM emission, and production nodes.
...
Latest verified operational product baseline: `4cada95ed2f97c2d0251dd82ef40290b0c664c41 fix: keep ThreatHint drains progressing (#79)`; Prometheus CI `29975041446`, Security Audit `29975041416`, and Pages `29975040944` pass. Latest verified public/status baseline is `e1756f966e66d92e00dbdeb161dd57641808ba4e` with CI `29975474205`, Security `29975474202`, Pages `29975473701`, and live README/Whitepaper/Roadmap/`llms.txt` checks.
Latest completed product task: issue https://github.com/NeaBouli/prometheus-/issues/77 is closed after PR https://github.com/NeaBouli/prometheus-/pull/79 merged normally. No approved production relation, verifying key, proving key, or vectors are bundled, and no analyzer indicators are fabricated from the hash-only transport schema. `PeerId` remains transport metadata only. Wallet/signing/chain behavior, KAS/PROM roles, slash ACL, commit-reveal, reputation, and emergency-stop policy remain unchanged.
