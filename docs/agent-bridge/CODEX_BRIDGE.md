...
Latest hardening note: deploy orchestrator-result imports and deployment receipt verification now reject raw/serialized transaction fields in addition to secret-like fields. Local py_compile, CI YAML parse, deploy receipt import/verification smoke, Prometheus CI, Security Audit, and Pages all passed for `181cde2`.
Current local work note: Added public node/explorer deployment receipt evidence verification. `scripts/verify_silverc_deploy_receipt_evidence.py` validates verified `operator_record` receipts against public evidence snapshots, rejects secret-like and raw/serialized transaction fields, and does not query nodes, accept keys, assemble, sign, broadcast, deploy, or update status files. `scripts/build_silverc_operator_handoff.py` now accepts `--deploy-receipt-evidence` and blocks handoff readiness when real operator receipts exist without matching public evidence. `scripts/audit_silverc_release_readiness.py` now requires and safety-validates the receipt-evidence summary before `ROLLOUT_READY`. Local py_compile, CI YAML parse, fresh release archive -> deploy requests -> external result import -> operator receipts -> public evidence verification -> handoff -> release-readiness smoke, and negative evidence/tamper tests passed on 2026-07-12. Remote CI still needs to be run for this local work.
...
- L-002 / Q-003: current-Silverc contract gate uses signed metrics `fp_rate` input; public metrics report preflight, unsigned tx-request builder, external oracle operator procedure, public external-operator capability verifier, public tx-result verifier, and public status-draft staging pass locally and in GitHub Prometheus CI for `181cde2`; external chain transaction assembly/signing/broadcast/deploy integration still needed.
