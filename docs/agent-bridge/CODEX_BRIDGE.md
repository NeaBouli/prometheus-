...
Latest verified operational baseline: `d0f78a9857e654dd487678a031d39ac52a44e0ec feat: authenticate guardian ballot intake (#40)`
Latest verified rollout/docs baseline: `d0f78a9857e654dd487678a031d39ac52a44e0ec feat: authenticate guardian ballot intake (#40)`
Latest active task: GH-42 implements the first real Guardian ballot transport vertical slice on branch `feat/GH-42-guardian-libp2p-carrier`. Merge, protected checks, exact-main evidence, and live Pages verification remain pending.
Latest CI note: exact main `d0f78a9` passed Prometheus CI `29464295373`, Security Audit `29464295355`, and Pages `29464294890`; all runs are bound to the same SHA. PR #43 passed all nine protected contexts in Prometheus CI `29467187250` and Security Audit `29467187280`; CodeRabbit then supplied five actionable hardening findings that are fixed locally and await the refreshed protected run. Current local evidence is 126 Guardian tests with three intentional live-model skips, Rustfmt, warning-free Clippy, 181 Rust workspace tests with two intentional live ignores, Black, CI-scope Pylint 9.95/10, dependency audits with no known vulnerabilities, and diff checks. Two workspace attempts hit the documented sub-millisecond performance jitter at 1.278 ms and 2.291 ms, the latter under concurrent Cargo artifact locks; isolated reruns completed in 44 and 43 microseconds and clean complete workspace reruns passed. Live Whitepaper, Roadmap, and GitHub README markers remain pending for the GH-42 merge. Rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open.
Current work note: issue https://github.com/NeaBouli/prometheus-/issues/42 tracks direct Guardian ballot request/response. The Rust carrier uses the pinned rust-libp2p 0.56 component set, QUIC, exact opaque frames capped at 8192 bytes, static peers, one stream per connection, global request/connection/time limits, and relay/AutoNAT/DCUtR behaviours. An owner-only AF_UNIX bridge resolves only locally registered sessions, invokes the unchanged GH-39 collector outside the asyncio event loop, and binds canonical local ACKs to the exact payload digest before returning a one-byte network status. `PeerId` remains transport metadata only. Independent review found an effective-UID ownership gap and medium availability/missing-bridge concerns; effective UID plus peer-credential checks, global/per-connection admission, thread offload, and an end-to-end QUIC-to-collector test were added. CodeRabbit subsequently found that a timed-out Python worker could release admission before its thread stopped, Python client ACK validation did not bind the returned digest, and collector I/O could pause exclusive swarm polling. Admission is now held by a shielded worker task until the underlying `to_thread` completes, non-busy ACKs are digest-bound, and bounded in-state Rust ingress futures are polled alongside the swarm. Follow-up review found peer-cancellation ordering races; separate work, active-ingress, and response-channel tracking now retains capacity through real completion and treats closed automatic response channels as nonfatal, with ordered and unordered cancellation regressions. Final independent re-review reports zero blocking/high/medium findings. The libp2p umbrella crate and mDNS were removed after optional DNS dependencies introduced unresolved lockfile advisories; direct component dependencies restore zero known `cargo audit` vulnerabilities. Operated relay/NAT/discovery, trusted membership/key assignment, Sybil resistance, on-chain attestation, and a packaged persistent-identity operator service remain open. No chain, wallet, contract, signing, reputation, KAS/PROM, slash ACL, commit-reveal, or GH-9 execution boundary changed.
