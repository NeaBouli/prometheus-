...
Operational baseline: exact main `83bdfe0e52e9308e28c8f0984a1219f203aa1f74` merged the GH-100 signal-readiness fix through protected PR #101 and passed Prometheus CI `29994190542`, Security Audit `29994190564`, and Pages `29994189428`. GH-94 previously merged the local mandatory-review `byte_pattern` producer through PR #97 as `34ab5b7a62b17ef2c9cab672439b77dcf4a66d9c`; raw GitHub and live Pages expose that boundary. All product, public-site, and status changes remain PR-only, and foreign untracked `Prometheus-1.png` remains untouched.
Current task: issue https://github.com/NeaBouli/prometheus-/issues/98 and branch `docs/GH-98-byte-pattern-producer-closeout-r2` reconcile GH-94 and GH-100 merged exact-main evidence across README, Whitepaper, Pages, Bridge, and Memory only. The documentation gate also corrected the public license record: the repository now includes the standard MIT `LICENSE` named by README and Whitepaper/`llms.txt` match the MIT declarations in Cargo and README. GH-94 remains one isolated Rust `byte_pattern` producer from exact bytes, checked offset, boolean wildcard mask, and typed scope. It accepts no path, pattern string, arbitrary observable value, or generic builder input; requires 8..=64 positions with at least eight fixed bytes; and always emits local-only `review_required_v1`. GH-100 only installs Unix SIGINT/SIGTERM listeners before sidecar operator output. Neither slice changes v1 wire, P2P authorization, verifier, analyzer, proof, wallet, signing, chain, KAS/PROM, slash ACL, commit-reveal, reputation, or emergency-stop policy.
...
Latest verified operational product and public/status baseline: `83bdfe0e52e9308e28c8f0984a1219f203aa1f74 fix: install signal listeners before readiness (#101)`; Prometheus CI `29994190542`, Security Audit `29994190564`, and Pages `29994189428` pass.
Latest completed task: issue https://github.com/NeaBouli/prometheus-/issues/100 is closed after PR https://github.com/NeaBouli/prometheus-/pull/101 merged normally. The Unix-only Guardian sidecar installs SIGINT/SIGTERM listeners before any operator record. GH-94 remains merged/exact-main verified as the local mandatory-review `byte_pattern` producer. No wallet, signing, chain, KAS/PROM, slash ACL, commit-reveal, reputation, or emergency-stop boundary changed.
