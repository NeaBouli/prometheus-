...
Latest verified operational baseline: `71e578342d26639740e76172245c47b8c4e6dbaf ci: bound required checks and refresh rollout memory (#31)`
Latest verified rollout/docs baseline: `e9a970a9d3dbaa98cd754a4149075c0cca866001 docs: sync exact-main H-001 handoff (#28)`
Latest in-progress task: obtain an explicitly approved external BIP340 signature response for the byte-identical `205e1ca` H-001 request, then run canonical import and complete transaction verification. Any one-shot broadcast is a separate explicit-approval gate. The first real metrics transition remains separately gated.
Latest CI note: exact operational main `71e5783` passed Prometheus CI `29457601210`, Security Audit `29457601183`, and Pages `29457600490`. All nine protected jobs ran under explicit bounds; the jointly pinned Security Audit environment passed, including Dependency Audit in 2m54s, and Current Silverc Runtime passed in 2m35s. Exact public-status commit `e9a970a` remains live in README, index, roadmap, and whitepaper with the `205e1ca` handoff, live UTXO revalidation, 68-72% core estimate, and unchanged external gates. Rollout issue https://github.com/NeaBouli/prometheus-/issues/9 remains open; GH-30 is closed.
Current PR note: CI-hardening PR https://github.com/NeaBouli/prometheus-/pull/31 merged normally as `71e5783`; all nine protected contexts passed, optional CodeRabbit remained pending without review threads, and no direct-main push or admin bypass was used. Execution artifact baseline remains `205e1ca`; the CI/docs-only merge does not invalidate the byte-identical signing request.
...
Current GH-33 note: issue #33 and branch `feat/GH-33-guardian-hybrid-routing` implement the first bounded Sprint 10B slice. The dependency-injected local router runs 8B first, keeps the exact `0.70` boundary on 8B, escalates below it to 70B, preserves the `0.85` submission threshold, and fails closed on invalid hashes, confidence, types, adapter returns, or escalation output. Independent Terra review found and rechecked threat-hash/runtime-type/adapter-return hardening. Local Guardian verification passes 47 tests with three intentional live-model skips, Black, and Pylint 9.69/10. Live model wiring/calibration, P2P, ensemble voting, PR checks, merge, and exact-main evidence remain open. The official `rusty-kaspa` master at `78257f273a26c4be085bab0f79437dee99ca8835` still contains the PSKT v1 compute-budget TODO and constructs `ComputeCommit::SigopCount`, so GH-9 remains on the external public BIP340 response path; wallet, key, signature, raw transaction, and broadcast boundaries are unchanged.
