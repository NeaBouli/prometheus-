...
Current task: issue https://github.com/NeaBouli/prometheus-/issues/58 is the next autonomous product slice: a dedicated owner-only ThreatHint verifier ingress with persistent freshness/replay admission and bounded analyzer handoff. The operated sidecar must remain fail-closed and return no `accepted` result until an independently approved real Groth16/KIP-16 relation, verifying key, and test vectors are wired. Ballot transport, `PeerId` authorization boundaries, wallet/signing/chain behavior, KAS/PROM roles, slash ACL, commit-reveal, and emergency-stop policy remain unchanged.
Latest verified operational baseline: `c8a6cb83419d442542257f470af35d76528786bc feat: add bounded ThreatHint transport core (#56)`
Latest verified public rollout/docs baseline: `a78d3ed7d3f05c41a9adfa3746aa2a9884dd60b0 docs: close out ThreatHint transport core (#57)`; Prometheus CI `29647112173`, Security Audit `29647112172`, Pages `29647111845`, and live GitHub README, Whitepaper, Bridge, Pages Whitepaper/Roadmap, and `llms.txt` markers pass
Latest completed task: issue https://github.com/NeaBouli/prometheus-/issues/55 is closed after PR https://github.com/NeaBouli/prometheus-/pull/56 merged normally. GH-55 provides the canonical bounded ThreatHint schema, development-only Light Client builder, independent libp2p request/ACK behavior, shared transport admission/stream budgets, and fail-closed operated sidecar. Accepted hint analysis remains blocked until dedicated owner-only real-Groth16 verification, freshness/replay persistence, and bounded analyzer admission exist. `PeerId` remains transport metadata only.
