...
- Exact-main Prometheus CI `29970159892` then exposed the same legitimate early exit in the all-zero relation-source-anchor preflight. The follow-up routes all four expected network/manifest preflight-unavailable invocations through the explicit helper and one shared exit-3/silent-output assertion; valid and invalid proof paths still reject every stdin write error.
