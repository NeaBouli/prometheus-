...
- Fixed the overload path with a separately bounded rejection semaphore. A rejected connection now reads and validates one exact bounded frame before answering `busy`, avoiding Linux RST without permitting unbounded reject tasks. The focused regression, all 32 unit tests, the process test, Rustfmt, and all-target Clippy pass locally; refreshed remote checks are pending.
