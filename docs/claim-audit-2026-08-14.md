...
|---|---|---|---|---|---|
| Light Client malware detection | README described on-device Phi-3 as part of the present system; module comments described ONNX inference as running | `modules/client/src/ai/phi3.rs` has no ONNX session and uses an entropy heuristic behind a development-only stub gate; `blockchain/krc20.rs` returns a cache; `network/zk_proof.rs` generates a SHA-256 placeholder | Implemented and tested development foundation; production detection blocked/unproven | Say explicitly that ONNX inference, canonical L1/IPFS rule loading, operated P2P reporting, and production proving are absent | README, Whitepaper, FAQ, roadmap, HTML, `llms.txt`, client status |
