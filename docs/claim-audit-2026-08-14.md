...
|---|---|---|---|---|---|
| Light Client malware detection | README described on-device Phi-3 as part of the present system; module comments described ONNX inference as running | `modules/client/src/ai/phi3.rs` has no ONNX session and uses an entropy heuristic behind a development-only stub gate; `blockchain/krc20.rs` returns a cache; GH-190 adds bounded local Raw-CIDv1/exact-byte ingestion with 26 adversarial integration tests; `network/zk_proof.rs` generates a SHA-256 placeholder | Implemented and tested development foundation; production detection blocked/unproven | Say explicitly that local caller-supplied ingestion is not canonical L1/IPFS loading, production YARA, operated P2P reporting, or production proving | README, Whitepaper, FAQ, roadmap, HTML, `llms.txt`, client status |
