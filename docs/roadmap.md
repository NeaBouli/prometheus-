...
- GH-242 is implemented and locally tested: local ballot-session establishment now owner-loads one GH-147 source exactly once, checks a separately trusted network and expected epoch, derives the snapshot and BIP340 signer map internally, and removes the public caller-constructed context registration path without changing ballot bytes, session digests, replay behavior, or ensemble math. Epoch is an identity pin only. External membership authority, key ownership/rotation, Sybil resistance, multi-host operation, L1 attestation, and production trust remain open
