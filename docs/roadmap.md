...
- GH-229/PR #230 merged at exact main `fba8bb4`: the tested repository capability preserves loopback as the default while one explicit Development/Testnet-10 route accepts a single canonical direct literal-IP/UDP/QUIC-v1 Guardian peer. Exact-main CI `33017195813`, Security Audit `33017196184`, and Pages `33017194744` pass. Unsafe/special ranges, IPv4-mapped IPv6, DNS, TCP, relay and peer mismatches fail closed. A committed non-authorizing boundary and strict redacted evidence gate are tested, but direct UDP between the approved hosts is currently closed; therefore no real two-host or public operation is claimed
