...
- A locally verified, not-yet-merged Windows PE review candidate adds the same isolated `api_import` boundary for PE32/PE32+ exact bytes; 16 MiB/4096-entry limits, fail-closed ordinal/grammar checks, internally fixed `windows`/`pe`, architecture-specific Rust coverage, and independent Python parsing of a synthetic PE32+ vector are complete; library names never become observables and no path/string/generic, transport, proof, analyzer, wallet, chain, or promotion behavior is added
