...
- GH-107 candidate adds matching local Rust/Python verification of one canonical, maximum-one-hour BIP340 approval statement bound to the exact review-required bundle, trusted approver key, recipient-scope digest, network, report nonce, and time; it adds no signer, replay ledger, transport, promotion, disclosure, analyzer, proof, wallet, or chain action
- Pending: remaining reviewed kind-specific extractors, durable one-time approval consumption and authority/policy management, then a separate v2 wire, statement/relation, approved production proof artifacts, owner-only pairing, and actionable analysis
