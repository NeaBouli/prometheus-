...
*Project status reviewed through 2026-09-06. The immutable public claim-audit baseline remains 2026-08-14 at exact main `5cd13bf`; later exact-main updates and the current GH-253 repository candidate are recorded below and in the machine-readable post-audit ledger.*
...
- GH-193/PR #194 is merged and exact-main verified at `d0cd087`, adding strict development-only decoding of exact caller-supplied current-Silverc `RuleStorageState` constructor JSON; provenance, covenant-instance authentication, finality, and live IPFS retrieval remain open
- GH-197/PR #198 is merged and exact-main verified at `28da2d4`, adding a development-only, separately owner-pin-hashed canonical Testnet-10 manifest check over a bounded caller-supplied RPC-shaped UTXO set before GH-193 decoding. Unique outpoint, covenant, script, amount, block-DAA, checked maturity-proxy, and exact constructor-byte bindings fail closed; live RPC acquisition, manifest authority, chain truth/history, finality, and IPFS availability remain open
- GH-203/PR #204 adds a development-only adapter over the existing connected client: bounded lock/RPC waits acquire exact Testnet-10 network/virtual DAA and one explicit address's UTXOs, then direct pinned-field conversion feeds GH-197. Independent RPC truth/history, consensus finality, manifest authority, IPFS availability, deployment, and production readiness remain open
- GH-205 composes one owner-pinned complete snapshot through the GH-203 observation boundary, a credential-free loopback-only local IPFS gateway fetch, exact Raw-CIDv1 content binding, and one atomic scanner replacement. GH-207 adds an owner-local restart-persistent POSIX checkpoint that rejects verified-virtual-DAA rollback and same-order equivocation and supports exact replay recovery. GH-209 adds an explicit opt-in development coordinator with bounded retry, timeout, cancellation, single-flight admission and non-sensitive status. GH-211 adds a strict canonical BIP340-authenticated complete-snapshot envelope using a separately owner-pinned x-only key, external nonzero minimum sequence, and separately trusted per-fetch clock. Merged and exact-main-verified GH-213/PR #214 at `bbe7efb` adds an explicit operator-invoked Development/Testnet-10 preflight/run CLI: strict private local files, offline non-mutating preflight, loopback-IP-literal RPC/IPFS, redacted counters, and signal cancellation; CI `31950806131`, Security `31950806118`, and Pages `31950805653` pass. It authenticates one owner-authorized snapshot envelope only; key authority/rotation, persistent sequence authority, autonomous distribution, canonical manifest authority, independent RPC truth/finality, IPFS availability/replication, production YARA, deployment, and production readiness remain open
- Merged and exact-main-verified GH-216/PR #217 at `13c1812` adds test-only real-binary loopback E2E evidence for offline/connected preflight, private checkpoint commit, TERM/INT drain, exact restart replay, rollback/equivocation rejection, and malformed/timeout/disconnected peers; CI `31978132036`, Security `31978132044`, and Pages `31978131647` pass. This is local Development evidence only, not public Testnet operation, independent RPC/IPFS truth or availability, deployment, Mainnet, or production readiness
- KRC-20 UTXO queries for "PROM-RULES" tick (real on-chain reads)
- Operated canonical rule discovery and replicated IPFS availability via CIDv1
...
- GH-177 merged and exact-main verified: one isolated offline evaluator scans only 20 bounded deterministic synthetic in-memory buffers with one fixed GH-173-shaped rule under pinned `yara-x==1.4.0`. Its canonical authority-`none` report binds exact corpus, policy, evaluator bytes, engine version, rule digest, confusion counts, and precision/recall/specificity. PR #178 merged as exact main `396d347`; CI, Security Audit, and Pages pass on that SHA. No real sample, file/process scan, governed worker/outbox/result wiring, model, disclosure, submission, wallet, chain, reward, deployment, real-world semantic-quality claim, or production authority is added
- GH-180 merged and exact-main verified: eight deterministic POSIX integration tests compose canonical synthetic transport bytes through real Python ingress, governed promotion, schema-v5 atomic acceptance/outbox, bounded worker, and a durable GH-173 non-actionable semantic-draft result. Exact bindings, malformed/oversized rejection, replay/restart, duplicate concurrency, lease recovery, redacted analyzer failure, and transactional rollback pass with 171 adjacent tests; 1303 Guardian tests passed and 4 intentional live-model tests skipped. PR #181 merged as exact main `a28ad00`; CI `31662874366`, Security Audit `31662874399`, and Pages `31662873670` pass. No product runtime, GH-177 wiring, real sample, network service, model, scan, actionable authority, deployment, or production certification is added
...
- Merged and exact-main-verified PR #227 published exact main `6c39af5`; Prometheus CI `32675287618`, Security Audit `32675287530`, and Pages `32675287300` pass. GH-226 implements one Development-only Light Client v1 ThreatHint sender over the existing Guardian P2P stack. Strict owner-local files, offline preflight, one static literal-loopback QUIC route, exact canonical bytes, bounded redacted outcomes, and real same-host binary/Guardian evidence are covered. This is not public Testnet, multi-host, proof, membership, deployment or production evidence
- GH-229/PR #230 merged at exact main `fba8bb4`: the tested repository capability preserves loopback as the default while one explicit Development/Testnet-10 route accepts a single canonical direct literal-IP/UDP/QUIC-v1 Guardian peer. Exact-main CI `33017195813`, Security Audit `33017196184`, and Pages `33017194744` pass. At `2026-08-26T23:36:04Z` (2026-08-27 operator-local), one operator-attested run from source commit `27e8b02` delivered one canonical hint between two distinct controlled hosts with `rejected`/`rejected`, zero retries, no persistence and acknowledgement authority `none`. The temporary sender-restricted UDP rule was removed immediately. The [redacted evidence](evidence/gh-229-controlled-two-host-2026-08-27.json) does not independently prove separation and is not public-network, relay/v2, deployment or production evidence
- Merged and exact-main-verified GH-234/PR #235, code commit `b450740`, exact main `f146fb2`, adds one Development/Testnet-10-only Light Client ThreatHint-v2 sender for an owner-prepared canonical shared payload. Parsing against separately trusted `testnet-10`, strict owner-only files, offline preflight, exact one-shot `/prometheus/threat-hint/2.0.0` delivery, bounded redacted acknowledgements, Beta/Mainnet fail-before-network gates and v1 regression are locally tested, including the real binary over same-host QUIC. Exact-main CI `33272578070`, Security Audit `33272577951`, and Pages `33272577407` pass. It provides no proof generation or approval authority, public/multi-host v2 evidence, deployment, Mainnet or production operation
...
- Merged and exact-main-verified GH-242/PR #243 at `5cb132c670d1e7771ccaf6dab2ddf5b1a6fd905a`: local ballot-session establishment now owner-loads one GH-147 source exactly once, checks a separately trusted network and expected epoch, derives the snapshot and BIP340 signer map internally, and removes the public caller-constructed context registration path without changing ballot bytes, session digests, replay behavior, or ensemble math. Exact-main CI `33433012614`, Security Audit `33433012605`, and Pages `33433011653` pass. Epoch is an identity pin only. External membership authority, key ownership/rotation, Sybil resistance, multi-host operation, L1 attestation, and production trust remain open
- Merged and exact-main-verified GH-246/PR #247 at `f12e821bb492caae3b94e5b3c882488eb7f2982d`: canonical BIP340-signed membership transitions bind network, exact previous/next source digests, strictly advancing epoch, bounded validity and nonce into an owner-only durable ledger. Rollback, same-epoch equivocation, replay, clock rollback, source substitution, restart and concurrent duplicates fail closed. New ballot sessions derive only the stored current source under the same ledger lock. CI `33452085421`, Security Audit `33452085419`, and Pages `33452084065` pass. This is owner-pinned public verification only; external/decentralized authority, key ownership/rotation, Sybil resistance, L1, multi-host and production trust remain open
- GH-253 local repository candidate: exact dual-signed BIP340 envelopes require the durable current authority to authorize a gapless successor and the proposed key to prove possession under an independent digest domain. Rotation binds current membership, validity and nonce; durable key/history/clock state, replay rejection, historic-key exclusion, restart behavior, schema-v1 migration and concurrent rotation/transition serialization are tested. Later membership transitions verify only the current durable key. This is owner-local succession consistency without a signer/private-key path; real-world key ownership, external/decentralized membership authority, Sybil resistance, L1, public multi-host operation, deployment and production trust remain open. Issue #253 awaits protected merge and exact-main evidence
...
- Pending: broad discovery, externally trusted decentralized membership authority, real-world key ownership, Sybil resistance, and on-chain ensemble attestation. GH-253 covers only owner-local transition-key succession mechanics
