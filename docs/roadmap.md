...
- GH-161: the preferred capture mode derives a canonical manifest digest from exact regular-file bytes in one bounded trusted-owner local model directory and re-verifies the directory before model adapter construction; the old caller digest remains legacy-only, while upstream authenticity and live served-model identity remain unproven
