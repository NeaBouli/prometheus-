...
- GH-103 adds one bounded local Rust Linux ELF `api_import` producer from exact bytes plus a checked sorted/deduplicated import index; parser input is capped at 16 MiB/4096 dynamic symbols, scope is derived internally, every result is `review_required_v1`, and Python independently parses shared exact-byte vectors; no path/string/generic builder, transport authorization, external provenance, privacy approval, or proof binding is added
