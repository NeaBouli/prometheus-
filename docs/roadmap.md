...
- GH-205 composes one owner-pinned complete snapshot through the GH-203 observation boundary, a credential-free loopback-only local IPFS gateway fetch, exact Raw-CIDv1 content binding, and one atomic scanner replacement. GH-207 adds an owner-local restart-persistent POSIX checkpoint that rejects verified-virtual-DAA rollback and same-order equivocation and supports exact replay recovery. GH-209 adds an explicit opt-in development coordinator with one immediate attempt, a fixed interval after success, sequential retries with capped exponential failure backoff, bounded timeout, cancellation, single-flight admission and non-sensitive status. It has no CLI/product-runtime wiring or autonomous authority; canonical manifest authority, independent RPC truth/finality, IPFS availability/replication, production YARA, deployment, and production readiness remain open
