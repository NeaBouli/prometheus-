...
- GH-205 composes one owner-pinned complete snapshot through the GH-203 observation boundary, a credential-free loopback-only local IPFS gateway fetch, exact Raw-CIDv1 content binding, and one atomic scanner replacement. GH-207 adds an owner-local restart-persistent POSIX checkpoint that rejects verified-virtual-DAA rollback and same-order equivocation and supports exact replay recovery. The path remains callable-only development plumbing; automatic updates, canonical manifest authority, independent RPC truth/finality, IPFS availability/replication, production YARA, deployment, and production readiness remain open
