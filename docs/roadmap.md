...
- GH-234/PR #235, code commit `b450740`, adds one Development/Testnet-10-only Light Client ThreatHint-v2 sender for an owner-prepared canonical shared payload. Parsing against separately trusted `testnet-10`, strict owner-only files, offline preflight, exact one-shot `/prometheus/threat-hint/2.0.0` delivery, bounded redacted acknowledgements, Beta/Mainnet fail-before-network gates and v1 regression are locally tested, including the real binary over same-host QUIC. It provides no proof generation or approval authority, public/multi-host v2 evidence, deployment, Mainnet or production operation
