...
| 4 | Guardian analyzer foundation plus local vLLM runtime | CORE ACCEPTED / GH-144 RUNTIME MERGED |
...
- GH-147 local candidate: an exact schema-v1 source loaded through an owner-only, no-symlink, bounded, descriptor-verified file boundary binds 5–1024 sorted unique Guardian IDs one-to-one to public BIP340 keys, fixed 8B tier, and model-artifact digests. The validated source derives both existing views: its exact-byte digest pins the membership snapshot, while validated member fields derive signer mappings. This proves structural assignment consistency only, not source authority, key ownership/rotation, Sybil resistance, chain attestation, or production readiness
...
- Pending: an independently reproducible real 8B/70B candidate run, independently verified model-artifact provenance, real adversarial semantic-quality evaluation, production calibration evidence, and authorization
