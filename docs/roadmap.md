...
- GH-229 repository candidate: loopback remains the default while one explicit Development/Testnet-10 route accepts a single canonical direct literal-IP/UDP/QUIC-v1 Guardian peer. Unsafe/special ranges, IPv4-mapped IPv6, DNS, TCP, relay and peer mismatches fail closed. A committed non-authorizing boundary and strict redacted evidence gate are tested, but direct UDP between the approved hosts is currently closed; therefore no real two-host or public operation is claimed
