...
- GH-246 local candidate, protected review pending: canonical BIP340-signed membership transitions bind network, exact previous/next source digests, strictly advancing epoch, bounded validity and nonce into an owner-only durable ledger. Rollback, same-epoch equivocation, replay, clock rollback, source substitution, restart and concurrent duplicates fail closed. New ballot sessions derive only the stored current source under the same ledger lock. This is owner-pinned public verification only; external/decentralized authority, key ownership/rotation, Sybil resistance, L1, multi-host and production trust remain open
