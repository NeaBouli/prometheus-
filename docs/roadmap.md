...
- GH-147 local candidate: exact schema-v1 owner-only membership source binds sorted unique Guardian IDs one-to-one to public BIP340 keys, fixed 8B tier, and model-artifact digests; the exact-byte digest derives existing snapshots and signer mappings. This proves structural assignment consistency only, not source authority, key ownership/rotation, Sybil resistance, chain attestation, or production readiness
