...
- GH-180 local candidate: eight deterministic POSIX integration tests compose canonical synthetic transport bytes through real Python ingress, governed promotion, schema-v5 atomic acceptance/outbox, bounded worker, and a durable GH-173 non-actionable semantic-draft result. Exact bindings, malformed/oversized rejection, replay/restart, duplicate concurrency, lease recovery, redacted analyzer failure, and transactional rollback pass locally with 171 adjacent tests; 1303 Guardian tests passed and 4 intentional live-model tests skipped. No product runtime, GH-177 wiring, real sample, network service, model, scan, actionable authority, deployment, or production certification is added; protected PR/CI/merge remain pending
