...
- GH-170 local candidate replaces substring-only candidate-rule checks with an exact-pinned, compile-only YARA-X boundary: one bounded ASCII rule, no imports/includes/modules, zero compiler errors or warnings, and no scan or external effect. This is structural validation only; merge, exact-main evidence, semantic/adversarial quality, actionable analysis, and production authority remain open
