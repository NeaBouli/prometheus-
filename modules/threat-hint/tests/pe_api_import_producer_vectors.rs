...
use prometheus_threat_hint::{
    produce_pe_api_import_bundle, DisclosurePolicy, ObservableBundle, ObservableKind, ScopeFormat,
    ScopePlatform,
};
...
fn vectors() -> ProducerVectors {
    serde_json::from_str(include_str!(
        "vectors/threat-observable-pe-api-import-producer-v1.json"
    ))
    .expect("valid producer fixture")
}

#[test]
fn exact_pe_bytes_produce_expected_review_required_imports() {
    ...
    let mut indexes = HashSet::with_capacity(fixtures.cases.len());
    let mut imports = HashSet::with_capacity(fixtures.cases.len());
    for case in fixtures.cases {
        ...
        assert!(
            indexes.insert(case.import_index),
            "duplicate producer import index"
        );
        assert!(
            imports.insert(case.api_import.clone()),
            "duplicate producer import value"
        );
        let expected_wire = decode(&case.wire_hex).expect("wire hex");
        assert_eq!(hex::encode(&expected_wire), case.wire_hex);

        let bundle = produce_pe_api_import_bundle(&artifact, case.import_index)
        ...
        assert_eq!(reparsed.scope().platform(), ScopePlatform::Windows);
        assert_eq!(reparsed.scope().format(), ScopeFormat::Pe);
