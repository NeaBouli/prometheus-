...
def _make_authority(directory: Path, source_path: Path) -> GuardianMembershipAuthority:
    key = PrivateKey((5_000).to_bytes(32, "big"))
    policy = directory / "membership-authority.toml"
    policy.write_text(
        "\n".join(
            (
                "schema_version = 1",
                f'network_id = "{_NETWORK}"',
                f'authority_xonly_public_key = "{key.public_key_xonly.format().hex()}"',
                f"bootstrap_epoch = {json.loads(source_path.read_bytes())['epoch']}",
                'bootstrap_membership_source_sha256 = "'
                + hashlib.sha256(source_path.read_bytes()).hexdigest()
                + '"',
                f'bootstrap_membership_source_path = "{source_path}"',
                f'ledger_path = "{directory / "membership-authority.sqlite3"}"',
                "",
            )
        ),
        encoding="ascii",
    ...
    policy.chmod(0o600)
    return GuardianMembershipAuthority(policy)
...
def _signed_transition_wire(
    ...
    unsigned = {
        ...
        "previous_membership_source_sha256": hashlib.sha256(
            previous_path.read_bytes()
        ).hexdigest(),
        "next_epoch": next_epoch,
        "next_membership_source_sha256": hashlib.sha256(
            next_path.read_bytes()
        ).hexdigest(),
        "not_before_ms": NOW_MS - 2_000,
        "not_after_ms": NOW_MS + 2_000,
        "nonce": hashlib.sha256(
            f"ingress-transition-{next_epoch}".encode()
        ).hexdigest(),
    }
    unsigned_wire = json.dumps(unsigned, separators=(",", ":")).encode("ascii")
    ...
    return json.dumps(
        {
            **unsigned,
            ...
            "signature": key.sign_schnorr(digest, aux_randomness=b"\0" * 32).hex(),
        },
        separators=(",", ":"),
    ).encode("ascii")
...
def make_ingress(tmp_path):
    ...
    ingress = BallotIngress(collector, _make_authority(tmp_path, path))
...
def test_establish_session_uses_stored_source_not_mutated_file(tmp_path) -> None:
    ...
    path.write_bytes(_canonical_source_bytes(_make_members(key_offset=100)))
    path.chmod(0o600)

    established = _establish(ingress, path, candidate)

    assert established.snapshot.membership_source_sha256 == original_digest
...
def test_expected_epoch_bounds_accepted(tmp_path) -> None:
    for epoch in (0, MAX_MEMBERSHIP_EPOCH):
        ...
        ingress = BallotIngress(collector, _make_authority(case_dir, path))
...
def test_expected_epoch_type_and_range_fail_before_file_access(
    ...
    path = _write_source(tmp_path, _canonical_source_bytes(_make_members()))
    authority = _make_authority(tmp_path, path)
    calls: list[tuple[str, int]] = []

    def fail_if_called(*, expected_network_id: str, expected_epoch: int):
        calls.append((expected_network_id, expected_epoch))
        raise AssertionError("source reader must not run")

    monkeypatch.setattr(authority, "current_source", fail_if_called)
    collector, _ = make_collector(tmp_path)
    ingress = BallotIngress(collector, authority)
    with pytest.raises(BallotIngressError) as caught:
...
def test_expected_epoch_mismatch_rejected_by_authority(tmp_path) -> None:
    path = _write_source(tmp_path, _canonical_source_bytes(_make_members()))
    collector, _ = make_collector(tmp_path)
    ingress = BallotIngress(collector, _make_authority(tmp_path, path))
    with pytest.raises(BallotIngressError) as caught:
...
def test_source_file_change_cannot_change_accepted_session(tmp_path) -> None:
    members_a = _make_members()
    path = _write_source(tmp_path, _canonical_source_bytes(members_a))
    candidate = _make_candidate()
    collector, _ = make_collector(tmp_path)
    ingress = BallotIngress(collector, _make_authority(tmp_path, path))
    established_a = _establish(ingress, path, candidate)

    members_b = _make_members(key_offset=100)
    raw_b = _canonical_source_bytes(members_b)
    path.write_bytes(raw_b)
    path.chmod(0o600)
    established_b = _establish(ingress, path, candidate)

    assert established_b == established_a
    assert len(ingress._contexts) == 1
    assert (
        established_b.snapshot.membership_source_sha256
        != hashlib.sha256(raw_b).hexdigest()
    )


def test_signed_transition_changes_only_future_session_membership(tmp_path) -> None:
    members_a = _make_members()
    bootstrap = _write_source(tmp_path, _canonical_source_bytes(members_a))
    authority_key = PrivateKey((5_000).to_bytes(32, "big"))
    authority = _make_authority(tmp_path, bootstrap)
    collector, _ = make_collector(tmp_path)
    ingress = BallotIngress(collector, authority)
    candidate = _make_candidate()
    established_a = _establish(ingress, bootstrap, candidate)

    next_dir = tmp_path / "next"
    next_dir.mkdir()
    members_b = _make_members(key_offset=100)
    next_source = _write_source(next_dir, _canonical_source_bytes(members_b, epoch=1))
    transition_wire = _signed_transition_wire(
        authority_key, bootstrap, next_source, 0, 1
    )
    authority.apply_transition(transition_wire, next_source, NOW_MS)
    established_b = _establish(ingress, next_source, candidate, epoch=1)
...
def test_distinct_sources_remain_session_and_signer_isolated(tmp_path) -> None:
    ...
    candidate = _make_candidate()
    collector_a, _ = make_collector(source_a_dir)
    collector_b, _ = make_collector(source_b_dir)
    ingress_a = BallotIngress(collector_a, _make_authority(source_a_dir, path_a))
    ingress_b = BallotIngress(collector_b, _make_authority(source_b_dir, path_b))

    established_a = _establish(ingress_a, path_a, candidate)
    established_b = _establish(ingress_b, path_b, candidate)
    ...
    assert ingress_a.process(forged_a, NOW_MS).status == "rejected"
    assert (
        ingress_a.process(sign_vote(context_a, 0).to_wire(), NOW_MS).status
        == "accepted"
    )
    assert (
        ingress_b.process(sign_vote(context_b, 0).to_wire(), NOW_MS).status
        == "accepted"
    )
    assert established_a.snapshot != established_b.snapshot
    assert established_a.session.session_id != established_b.session.session_id
    assert set(ingress_a._contexts) == {established_a.session.session_id}
    assert set(ingress_b._contexts) == {established_b.session.session_id}
...
def test_missing_source_file_rejected_redacted(tmp_path) -> None:
    ...
    key = PrivateKey((5_000).to_bytes(32, "big"))
    policy = tmp_path / "membership-authority.toml"
    policy.write_text(
        "\n".join(
            (
                "schema_version = 1",
                ...
                f'authority_xonly_public_key = "{key.public_key_xonly.format().hex()}"',
                "bootstrap_epoch = 0",
                f'bootstrap_membership_source_sha256 = "{"0" * 64}"',
                f'bootstrap_membership_source_path = "{missing}"',
                f'ledger_path = "{tmp_path / "membership-authority.sqlite3"}"',
                "",
            )
        ),
        encoding="ascii",
