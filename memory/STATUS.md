...
Repo current-Silverc operator handoff package: `scripts/build_silverc_operator_handoff.py` builds a public handoff directory from a release archive, deploy preflight, external deploy request set, CI fixture receipt verification, optional real operator receipt verification, metrics report preflight, and unsigned metrics-oracle tx request. It emits `HANDOFF.md` and `operator-handoff-summary.json`, keeps status `HANDOFF_BLOCKED` until real deploy tooling/receipts/instance IDs exist, and does not accept keys, sign, broadcast, deploy, or update status files.
