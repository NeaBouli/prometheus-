...
Repo current-Silverc deploy operator procedure: `scripts/build_silverc_deploy_operator_procedure.py` converts the verified deploy request set into a public external deploy checklist and required result-evidence contract. It reuses release-bundle/request validation, emits per-contract request hashes plus required public `operator_record` result fields, and publishes the official covenant-genesis profile: transaction version 1, `kaspa_txscript::pay_to_script_hash_script` over the compiled contract script, `kaspa_consensus_core::hashing::covenant_id` over the funding outpoint and unbound genesis outputs, and funding-input binding after ID derivation. `scripts/verify_external_operator_capability.py` requires an exact capability attestation to this profile. PR #2 merged normally as `9d74c0c`; Prometheus CI `29184186551`, Security Audit `29184186538`, and Pages `29184186085` passed on `main`. Neither script accepts keys, raw transactions, signing material, deployment, or status writes.
