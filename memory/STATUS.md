...
Repo current-Silverc deploy operator procedure: `scripts/build_silverc_deploy_operator_procedure.py` converts the verified deploy request set into a public external deploy checklist and required result-evidence contract. It reuses release-bundle/request validation, emits per-contract request hashes plus required public `operator_record` result fields, and publishes the official covenant-genesis profile: transaction version 1, `kaspa_txscript::pay_to_script_hash_script` over the compiled contract script, `kaspa_consensus_core::hashing::covenant_id` over the funding outpoint and unbound genesis outputs, and funding-input binding after ID derivation. `scripts/verify_external_operator_capability.py` requires an exact capability attestation to this profile. Local positive and transaction-version/P2SH/covenant-ID/binding tamper checks pass on branch `feature/GH-1-silverc-genesis-capability`; all 10 checks on PR #2 at `57617ac` pass, including Current Silverc Runtime, Gitleaks, cargo audit, and pip audit. Merge awaits an independent review and is not being bypassed. Neither script accepts keys, raw transactions, signing material, deployment, or status writes.
...
Sprint 9 remains blocked until the missing approved network deploy/orchestration path, real public deploy results/receipts plus public node/explorer evidence from that path, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and public release-hardening evidence for the exact rollout commit are proven. Current-Silverc runtime gates, release-bundle manifest/archive, deploy preflight, operator runbook, external deploy request set/verifier, public deploy operator procedure, public orchestrator-result receipt import with raw-transaction rejection, deployment receipt verifier, public receipt-evidence verifier, deployment status staging guard, operator handoff package, release-readiness audit, public metrics-oracle report preflight, unsigned oracle tx-request builder, external oracle operator procedure, public external-operator capability verifier, public oracle tx-result verifier, public oracle tx-evidence verifier, public oracle status-draft staging, and public release-hardening evidence verification pass locally and in CI for `40bb9a0`.
