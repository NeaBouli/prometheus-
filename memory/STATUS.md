...
| Silverscript tooling (silverc/ssc) | IN_PROGRESS | 90%      | 2026-07-09  | -            | Upstream `silverc` builds/tests in CI; H-001 fixture verifies; ValidatorStaking state fixture compiles; `commitVote` runtime transition tests pass; remaining transitions pending |
...
Repo ValidatorStaking runtime gate: `scripts/verify_silverc_h001.py` now injects upstream runtime tests for `commitVote`; valid bond/signature/state transition is accepted and low bond is rejected in GitHub Prometheus CI for `f1dd616`.
...
Sprint 9 remains blocked until the remaining current-Silverscript validator transitions (`revealVote`, `slashInvalidReveal`, `requestWithdraw`, `completeWithdraw`) have runtime tests with real signatures/authorized outputs, the signed-int/u64 boundary is resolved, and the remaining Prometheus contracts are ported or deployment-scoped against current Silverscript tooling.
...
Priority tasks:
- Sprint 9: add runtime transition tests for `ValidatorStakingState.sil` reveal/slash/withdraw paths, then port/compile remaining deployment-scoped contracts with current `silverc`
