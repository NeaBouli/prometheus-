...
| GitHub Actions CI/CD         | ACCEPTED        | 100%     | 2026-07-11  | ACCEPTED     | Prometheus CI, Security Audit, and Pages green for df4f8df; current-Silverc runtime, release-bundle manifest/archive, deploy preflight, operator runbook, external deploy request set/verifier, deployment receipt verifier, deployment status staging guard, operator handoff package, metrics-oracle report preflight, and unsigned oracle tx-request gates are CI-verified; workflow actions use Node 24-compatible majors |
