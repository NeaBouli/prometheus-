...
Repo current-Silverc deploy operator procedure: `scripts/build_silverc_deploy_operator_procedure.py` converts the verified deploy request set into a public external deploy checklist and required result-evidence contract. It reuses release-bundle/request validation, emits per-contract request hashes plus required public `operator_record` result fields, rejects secret-like fields through the shared request verifier, and does not accept keys, raw transactions, signing material, deployment, or status writes. Local flow and CI passed on 2026-07-11 for `a86c1b5`.
...
Repo current-Silverc operator handoff package: `scripts/build_silverc_operator_handoff.py` builds a public handoff directory from a release archive, deploy preflight, verified external deploy request set, deploy operator procedure, optional public orchestrator-result receipt import, CI fixture receipt verification, optional real operator receipt verification, optional public receipt-evidence verification, metrics report preflight, unsigned metrics-oracle tx request, optional external oracle operator procedure, optional verified external-operator capability record, optional verified oracle tx result, and optional oracle status draft. It emits `HANDOFF.md` and `operator-handoff-summary.json`, keeps status `HANDOFF_BLOCKED` until real deploy tooling/receipts/public evidence/instance IDs exist, and does not accept keys, raw transactions, sign, broadcast, deploy, or update status files.
Repo current-Silverc release-readiness audit: `scripts/audit_silverc_release_readiness.py` validates generated public operator handoff packages before any rollout claim. It checks required files, included-file consistency, handoff/component statuses, optional public receipt-evidence files, optional external-operator capability files, safety flags, and JSON secret/raw-transaction hygiene; it emits `ROLLOUT_BLOCKED` while real deploy/orchestration or external oracle-operation evidence is missing, and `--require-ready` fails until the blockers are cleared. It does not accept keys, raw transactions, sign, assemble, broadcast, deploy, or update status files.
...
Sprint 9 remains blocked until the missing approved network deploy/orchestration path, real public deploy results/receipts plus public node/explorer evidence from that path, external signed metrics-oracle transaction assembly/signing/broadcast/deploy operation, and final release hardening are proven. Current-Silverc runtime gates, release-bundle manifest/archive, deploy preflight, operator runbook, external deploy request set/verifier, public deploy operator procedure, public orchestrator-result receipt import with raw-transaction rejection, deployment receipt verifier, public receipt-evidence verifier, deployment status staging guard, operator handoff package, release-readiness audit, public metrics-oracle report preflight, unsigned oracle tx-request builder, external oracle operator procedure, public external-operator capability verifier, public oracle tx-result verifier, and public oracle status-draft staging pass locally; remote CI baseline remains `181cde2` until the receipt-evidence work is pushed.
...
- Oracle: integrate the external signed metrics-oracle transaction assembly/signer/broadcast path for GovernanceAutoTuning before beta/mainnet governance; public report preflight, unsigned tx-request builder, public tx-result verifier, and public status-draft staging are local/CI-covered
