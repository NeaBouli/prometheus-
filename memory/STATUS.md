...
Repo current-Silverc operator handoff package: `scripts/build_silverc_operator_handoff.py` builds a public handoff directory from a release archive, deploy preflight, verified external deploy request set, deploy operator procedure, optional public orchestrator-result receipt import, CI fixture receipt verification, optional real operator receipt verification, metrics report preflight, unsigned metrics-oracle tx request, optional external oracle operator procedure, optional verified external-operator capability record, optional verified oracle tx result, and optional oracle status draft. It emits `HANDOFF.md` and `operator-handoff-summary.json`, keeps status `HANDOFF_BLOCKED` until real deploy tooling/receipts/instance IDs exist, and does not accept keys, raw transactions, sign, broadcast, deploy, or update status files.
Repo current-Silverc release-readiness audit: `scripts/audit_silverc_release_readiness.py` validates generated public operator handoff packages before any rollout claim. It checks required files, included-file consistency, handoff/component statuses, optional external-operator capability files, safety flags, and JSON secret/raw-transaction hygiene; it emits `ROLLOUT_BLOCKED` while real deploy/orchestration or external oracle-operation evidence is missing, and `--require-ready` fails until the blockers are cleared. It does not accept keys, raw transactions, sign, assemble, broadcast, deploy, or update status files.
