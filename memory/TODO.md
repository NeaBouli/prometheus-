...
- [~] [P1] Implement ensemble voting protocol: 5+ 8B Guardians vote on same YARA rule via majority | Codex | GH-36 local fail-closed protocol merged/exact-main verified at `f8ebaac`; GH-39 locally adds exact per-session BIP340 key binding, strict canonical envelopes/freshness, and owner-only SQLite restart/concurrency/clock-rollback-safe replay and equivocation protection. Actual P2P carrier/discovery/NAT traversal, trusted membership/key assignment, Sybil resistance, on-chain attestation, and production evidence remain
