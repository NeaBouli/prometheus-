...
- [x] [P1] GH-216 binary-level RuleStorage sync loopback E2E evidence | Codex Sol + Kimi K3 | PR #217 merged as exact main `13c1812`; CI `31978132036`, Security `31978132044`, and Pages `31978131647` pass; test-only real-binary Borsh wRPC/IPFS loopback peers cover preflight/run, private checkpoint, TERM/INT drain, restart/replay/anti-downgrade and bounded adversarial failures; no real-node/Testnet operation, production source, key authority, wallet, chain, deployment, Mainnet or production claim
...
- [~] [P2] P2P-Grundgeruest (p2p.rs mit libp2p) | Kimi K3 + Codex | GH-226 schliesst den Development-only v1 Einzelsender; GH-234/PR #235 ist als exact main `f146fb2` verifiziert und ergaenzt den Development/Testnet-10-only v2 Einzelsender fuer owner-prepared canonical payloads mit realem same-host Binary/QUIC-Test. GH-238/PR #239 ist normal gemergt und als exact main `912d96d` verifiziert (CI `33279351831`, Security `33279351822`, Pages `33279351387`): repository-only Vorbereitung fuer einen spaeteren kontrollierten Distinct-Host Development/Testnet-10 ThreatHint-v2 Versuch ist implementiert und lokal getestet (challenge-gebundene rollenspezifische Operator-Attestierungen, strikte owner-only/no-symlink Dateien, exakte 9.265-Byte Rust Wire-Grenze, atomarer no-clobber Record, geschlossener redaktierter Verifier, CI-Tests); kein echter GH-238 Remote-Lauf und kein GH-238 Evidenz-Record existiert, Host-Trennung ist nicht unabhaengig bewiesen, und ein spaeterer echter Lauf erfordert eine separate explizite Autorisierung. Oeffentliches Multi-Host-v2-Netz, Membership/Discovery, Proof-/Approval-Autoritaet und Produktion bleiben offen
...
- [~] [P1] Implement ensemble voting protocol: 5+ 8B Guardians vote on same YARA rule via majority | Codex | GH-36/39 plus merged/exact-main GH-42/GH-44/GH-48/GH-52 are implemented. GH-242 binds establishment to GH-147; merged/exact-main GH-246/PR #247 at `f12e821bb492caae3b94e5b3c882488eb7f2982d` adds owner-pinned signed transition continuity and durable anti-rollback/current-source enforcement. Externally trusted authority, key ownership/rotation, public/multi-host operation, broad discovery, Sybil resistance, on-chain attestation, and production evidence remain
...
- [x] [P1] GH-242 bind local ballot sessions to the canonical membership source | Codex Sol + Kimi K3 | Product PR #243 merged normally as exact main `5cb132c670d1e7771ccaf6dab2ddf5b1a6fd905a`; CI `33433012614`, Security `33433012605`, and Pages `33433011653` pass. Public closeout PR #244 merged as exact main `d9b275452e70717372747ef9359abaec956fcf49`; CI `33435673539`, Security `33435673520`, and Pages `33435672362` pass with live public readback. Owner-load once; separately trusted network/epoch; internal snapshot/signer derivation; no public caller-context registration; unchanged ballot wire, replay and ensemble math. Production authority remains false
...
- [x] [P1] GH-253 owner-local Guardian transition-authority rotation | Codex Sol + Kimi K3 | PR #254 merged normally as exact main `5920cb4bb737376977f762beb0d5e3108519c7a0`; CI `34031999904`, Security Audit `34031999907`, and Pages `34031999575` pass. Dual BIP340 current-key authorization/new-key possession, gapless durable authority epochs, v1-to-v2 ledger migration, current-key-only later transitions and adversarial concurrency coverage are verified; real-world key ownership, external/decentralized authority, Sybil resistance, L1, deployment and production remain excluded
- [~] [P1] GH-258/GH-261 Endpoint Detection & Safe Response | Core Dev + independent security/privacy review | GH-264 repository candidate implements only the first canonical observe-only data contract in Rust/Python with no endpoint collection, sensor, correlation, warning, transport or response authority. GH-267 adds a machine-readable pre-producer privacy/threat-model and CI drift-gate candidate, but no producer, runtime collection, privacy proof or production authority. The wider track remains planned: opt-in least-privileged producers for process/resource ownership, unexpected CPU/GPU workloads, scheduler/orchestrator drift, workload identity, persistence, credentials and outbound fan-out; local warning and evidence; operator-confirmed reversible containment; separately approved limited automation. Behavior-based detection only, with no reliable AI/AGI/actor attribution. Automatic quarantine/process kill/firewall/credential rotation/remote command/deletion/host isolation remains unimplemented and unauthorized. Requires independent privacy review before a separately approved producer, then real-sample/adversarial, false-positive, multi-host and rollback evidence before any response authority
