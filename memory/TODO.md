...
- [~] [P1] Local review-required Linux ELF `api_import` producer | Codex + Claude Code + Terra review | GH-103 branch derives one import from exact ELF bytes plus a checked sorted/deduplicated index; 16 MiB/4096-symbol parser bounds, internally fixed scope, pinned read-only parser, shared Rust/Python exact-byte vectors, no path/string/generic builder or transport/proof/analyzer/chain wiring; protected PR and exact-main evidence remain
