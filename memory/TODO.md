...
- [~] [P1] Light Client ↔ Guardian P2P Kommunikation | Codex | GH-55 canonical schema/client builder/direct P2P core, GH-58 durable ingress, and GH-63's real active-KIP-16 BN254/Arkworks verification engine plus bounded owner-aware adapter/service are merged/exact-main verified. GH-74 adds the bounded digest/network/time-bound analyzer adapter on its feature branch; hash-only v1 input is consumed as an exact non-submittable no-LLM/no-YARA result. Operated acceptance remains fail-closed `busy` until independently approved production relation/VK/vectors are installed; actionable analysis also requires a reviewed concrete-observable channel or future schema
