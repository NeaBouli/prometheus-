...
- [x] [P1] Local review-required Windows PE `api_import` producer | Codex Sol + Kimi implementation/review | GIO-20260726-004 is locally Done on exact-main baseline `12a08d4`: exact caller-supplied PE32/PE32+ bytes plus checked sorted/deduplicated import index, internally fixed `windows`/`pe` scope, 16 MiB/4096-entry bounds, ordinal, malformed-table, and bound-IAT-without-lookup-table fail-closed handling, synthetic shared vectors, independent Python validation, two-library descriptor/deduplication coverage, and positive unbound-IAT fallback coverage; exact-main Kimi review found no high/medium issue, its bound-IAT low finding and fallback-coverage P3 were remediated, and the targeted remediation review passed; no paths, arbitrary strings, pairing, proof, transport, analyzer, wallet, chain, or promotion wiring; commit/push/PR/remote CI remain separately authorized publishing gates
