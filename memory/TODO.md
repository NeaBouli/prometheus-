...
- [~] [P1] Local `file_sha256` producer from exact artifact bytes | Codex + helper reviewers | GH-90 branch candidate computes one digest internally from bytes plus typed scope, exposes no path/digest/generic builder input, and has shared Rust-producer/Python-validator vectors; protected PR and exact-main evidence remain
