...
- [~] [P1] GH-213 opt-in Development/Testnet-10 RuleStorage sync CLI | Codex Sol + Kimi K3 | Issue #213 / branch `feat/GH-213-rule-sync-cli`; strict owner-local config/envelope files, offline non-mutating preflight, loopback-IP-literal RPC/IPFS factories, explicit run, redacted status and signal cancellation; local focused tests/review/full protected delivery remain; no key governance, wallet, chain write, deployment, Mainnet or production authority
