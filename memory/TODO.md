...
- [~] [P0] libp2p Modul komplett implementieren | Codex | GH-42 merged/exact-main verified; GH-44 candidate adds owner-only persistent identity, strict bounded routes, relay service, health events, and isolated three-node relay/AutoNAT/DCUtR-fallback/disconnect evidence. Protected merge, public/multi-host operation, broad discovery, and remaining client/validator paths stay open; mDNS remains excluded by the unresolved compatible RustSec dependency path
...
- [~] [P1] Implement ensemble voting protocol: 5+ 8B Guardians vote on same YARA rule via majority | Codex | GH-36/39 plus GH-42 are merged/exact-main verified. GH-44 persistent identity and isolated relay/NAT evidence are candidate-complete; protected merge, public/multi-host operation, broad discovery, trusted membership/key assignment, Sybil resistance, on-chain attestation, and production evidence remain
