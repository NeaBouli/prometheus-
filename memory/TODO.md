...
- [~] [P1] Regel-Download von IPFS via CIDv1 | Codex Sol + Kimi K3 | GH-190 merged through protected PR #191 as exact-main-verified `9c2faff` and implements bounded local Raw-CIDv1/exact-byte verification, strict simple-matcher parsing and atomic snapshot activation; canonical L1 reads, IPFS network fetching, production YARA and durable anti-downgrade state remain
...
- [~] [P1] GH-193 strict RuleStorage constructor-state decoder | Codex Sol + Kimi K3 | Protected PR #194 is open with local and PR CI/Security checks passing; exact-main merge and Pages evidence remain pending. Canonical RPC provenance, covenant-instance/finality proof and live IPFS retrieval remain separate tasks
