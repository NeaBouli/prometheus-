...
- [~] [P2] Replace heuristic YARA confidence with strict model-provided integer basis points | Codex Sol | GH-135 local candidate removes indicator-count/YARA-shape scoring, validates a separate exact closed-schema assessment, preserves the `0.85` policy, and has 149 focused passes/4 intentional live-model skips; complete gates, protected publication, live semantic/adversarial evaluation, and calibration remain
