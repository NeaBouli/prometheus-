...
- [~] [P2] Add deterministic Guardian confidence evaluation/calibration gate | Codex Sol | GH-138 local candidate internally consistency-checks a canonical 24-case synthetic corpus, exact predictions, fixed policy, expected report, and co-versioned manifest; this is not an external tamper anchor. 29 focused tests and byte-exact CLI reproduction pass. Live-model semantic quality, real adversarial evidence, production calibration, and authorization remain separate gates
