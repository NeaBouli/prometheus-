...
| Letzter grün verifizierter Produkt-/Tooling-Commit | `5224c009` — Prometheus CI `29468717108`, Security `29468717112`, and Pages `29468716410` green; GH-42 Guardian ballot carrier is merged/exact-main/live verified |
| Aktuelle Tooling-Baseline | Exact main `5224c009` passed all nine protected jobs plus Pages. GH-39 supplies BIP340/replay safety; merged GH-42 adds a bounded, cancellation-safe direct QUIC Guardian carrier and owner-only collector bridge without turning PeerId into Guardian authorization. GH-44 tracks persistent transport identity and operated relay/NAT evidence; broad discovery, trusted membership/key assignment, Sybil resistance, and on-chain attestation remain open. Execution artifacts remain bound to `205e1ca`; GH-9 remains ready for explicitly approved external signing. |
...
| Whitepaper | WHITEPAPER.md (root) + whitepaper.html (styled); miner companion, reward boundaries, GH-36, and GH-39 authenticated-intake boundaries are live-verified 2026-07-16 |
...
- [~] Sprint 10B ensemble voting: GH-36/39 local voting and authenticated intake plus GH-42 direct QUIC ballot transport and owner-only collector integration are merged/exact-main verified. GH-44 tracks persistent identity and operated relay/NAT evidence; broad discovery, trusted membership/key assignment, Sybil resistance, and on-chain attestation remain
...
*Prometheus v4.0 · Checkpoint 2026-07-16 · Last updated: operational main `5224c009` exact-main CI/Security/Pages green + GH-36 local fail-closed 5+ complete-ballot gate merged + GH-39 per-session BIP340 intake and persistent restart/concurrency/clock-rollback-safe replay/equivocation protection merged + GH-42 bounded cancellation-safe direct QUIC ballot carrier and owner-only collector bridge merged/exact-main/live verified while operated relay/NAT, membership, Sybil, and on-chain evidence remain open under GH-44 and later slices + GH-30 bounded required-check runtimes and pinned audit environment complete + H-001 handoff rebuilt/live-preflight verified from exact execution baseline `205e1ca` with byte-identical schema-v2 request + GH-25 keyless value-preserving reportMetrics operator merged/exact-main green at `072f04a` + external signatures/broadcast/evidence pending + GH-13 experimental local miner companion accepted at `2e4b4ec` + seven current-Silverc runtime gates + crash-consistent keyless Toccata-v1 genesis operator + manifest-bound non-promotable H-001 testnet-10 profile + TLS-only official resolver probe + public receipts/evidence/status staging + exact-commit release-hardening gates · The fire belongs to humanity.*
