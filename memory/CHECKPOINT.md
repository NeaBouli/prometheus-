...
| Letzter grün verifizierter Produkt-/Tooling-Commit | `f8ebaac` — Prometheus CI `29461803530`, Security `29461803531`, and Pages `29461802700` green; GH-36 local ensemble gate and public docs are exact-main verified |
| Aktuelle Tooling-Baseline | All nine protected jobs are explicitly bounded and the pinned audit environment is exact-main verified. GH-36 adds the local fail-closed 5+ complete-ballot gate; trusted membership, signed P2P/replay protection, Sybil resistance, and on-chain attestation remain open. Execution artifacts remain bound to `205e1ca`; GH-9 public funding/live UTXO and the byte-identical schema-v2 signing request are ready for explicitly approved external signing. |
...
| Whitepaper | WHITEPAPER.md (root) + whitepaper.html (styled), miner companion, reward boundaries, and GH-36 local ensemble boundary live-verified 2026-07-16 |
