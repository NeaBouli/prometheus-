...
| Aktuelle Tooling-Baseline | All nine protected jobs are bounded and the pinned audit environment is exact-main verified. GH-39 supplies BIP340/replay safety; GH-42 adds a bounded direct QUIC Guardian carrier and owner-only collector bridge without turning PeerId into Guardian authorization. Operated relay/NAT/discovery, trusted membership/key assignment, Sybil resistance, and on-chain attestation remain open. Execution artifacts remain bound to `205e1ca`; GH-9 remains ready for explicitly approved external signing. |
...
- [~] Sprint 10B ensemble voting: GH-36/39 local voting and authenticated intake are merged/exact-main verified; GH-42 direct QUIC ballot transport and owner-only collector integration are implemented. Operated relay/NAT/discovery, trusted membership/key assignment, Sybil resistance, and on-chain attestation remain
