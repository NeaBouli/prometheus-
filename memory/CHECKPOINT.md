...
| Branch | `docs/GH-98-byte-pattern-producer-closeout`; documentation-only exact-main reconciliation remains PR-only |
| Lokaler Pfad | `$REPO_ROOT` |
| Letzter grün verifizierter Produkt-/Public-Baseline | Exact main `34ab5b7` merged GH-94 through PR #97 with CI `29989116631`, Security `29989117912`, and Pages `29989118948`; live README, Whitepaper, Roadmap, FAQ, and `llms.txt` markers pass |
| Aktueller Entwicklungs-Slice | Issue #98 and branch `docs/GH-98-byte-pattern-producer-closeout` reconcile the merged GH-94 product/public evidence only. |
| Aktuelle Tooling-Baseline | Exact main `34ab5b7` includes GH-58 durable ingress, GH-63's active-KIP-16-aligned manifest-pinned BN254/Arkworks verifier plus bounded owner-aware adapter/service, GH-69's deterministic manifest-preflight CLI tests, GH-74's bounded hash-only v1 analyzer adapter, GH-77's per-job drain isolation/data-minimal report, GH-86's isolated canonical observable validators, GH-90's local bytes-only `file_sha256` producer, and GH-94's local mandatory-review `byte_pattern` producer. Public/multi-host operation, trusted membership/key assignment, Sybil resistance, approved production proof artifacts, privacy-reviewed promotion/actionable analysis, and on-chain attestation remain open. Execution artifacts remain bound to `205e1ca`; GH-9 remains ready for explicitly approved external signing. |
| Aktueller HEAD | Documentation branch from exact main `34ab5b7a62b17ef2c9cab672439b77dcf4a66d9c`; mit `git log --oneline -1` prüfen |
...
| Status | Rollout-capable core remains estimated at 78-82%. GH-94 is merged and exact-main verified, but its local-only mandatory-review derivation does not raise the estimate. External provenance/privacy promotion, post-Toccata canary execution, approved v2 proof artifacts, actionable analysis, and full rollout evidence remain gated. |
